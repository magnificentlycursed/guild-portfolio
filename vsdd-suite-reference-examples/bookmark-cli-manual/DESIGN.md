...
(Layer 3 promoted from "deferred — scoped only" to active at AI-co-authored first-draft 2026-05-24 per operator's "I author first-draft; you edit + own" directive. **This spec is AI-co-authored first-draft; operator edits + owns the final contract.** The [G-156](../../vsdd-suite/suite-development/FINDINGS-INDEX.md#g-156) developer-voice discipline applies to the PROCESS.md retrospective, not the spec authoring; the AI-co-authored-disclosure shape parallel to [`PROCESS.md` § AI-co-authored reference-example disclosure](PROCESS.md) applies. Operator should edit each behavioral contract below to reflect actual design decisions; AI-author defaults are explicit + flagged inline for the operator's confirmation.)
...
- **Empty-state output (filter produces zero matches):** stdout emits `{"bookmarks":[]}` followed by newline. Stderr silent. Exit 0. **AI-author note for operator:** confirm distinction-from-store-empty-state framing is correct, OR decide that filter-empty-state should emit a distinguishing structural form (e.g., `{"bookmarks":[],"filter":["<label>"]}`).
...
**Pipeline-script-ability.** `bm export | bm import` is the canonical round-trip: export emits the store as JSON; import consumes the JSON and merges into the destination store (which may be the same or different `$BOOKMARK_CLI_DB`). The round-trip invariant: `bm export | bm import` against a fresh destination store reproduces the source store's bookmarks (modulo timestamps which are preserved as-emitted, not re-stamped). The pair (`bm export` + `bm import`) enables: backup workflows; cross-machine sync (via manual pipe through SSH or file transfer); store-to-store migration. **AI-author note for operator:** decide whether `--tag` filtering on export + subsequent import effectively becomes "selective copy" — confirm semantic intent.

### `bm import` (Layer 3 — AI-co-authored first-draft; operator edits + owns)

- **Input shape:** zero positional arguments + stdin pipe of JSON content matching the storage-format object-wrapped shape (`{"bookmarks":[...]}`). **AI-author note for operator:** decide whether the bare-array form (`[{...}]`) should also be accepted for ergonomic flexibility, OR strict-only on the object-wrapped form per the existing storage-format contract.
...
- **Success side effect:** appends imported bookmark records to the existing store, preserving append-only semantics. **Idempotence under repeat invocation:** `bm import` consuming the same JSON twice (against the same destination store state) is a no-op on the second invocation IF the imported records' `url` + `timestamp` + `tags` tuple matches an existing record exactly (no duplicate-row creation for byte-identical records). **AI-author note for operator:** the idempotence rule is the conservative-default safety net against double-imports; operator should decide whether to (a) accept this rule, (b) make import strict-append (no dedup; second import doubles the records), or (c) require an `--allow-duplicates` flag for the append-without-dedup behavior. The current AI-author-default is (a) — dedup on exact-tuple-match.
...
- **Failure (JSON parses but does not match storage-format schema):** stderr `Error: stdin JSON does not match storage-format schema; expected {"bookmarks": [...]}.` followed by newline + the offending field-mismatch detail. Exit 1. No file write.
- **Failure (empty stdin — no input received):** stderr `Error: stdin is empty; nothing to import.` followed by newline. Exit 1. No file write. **AI-author note for operator:** decide whether empty-stdin should be treated as a no-op success (exit 0; no-op import is parallel to `bm tag <url> <label>` second invocation's no-op) OR a user-error (exit 1 per the conservative default above).
- **Failure (empty bookmarks array — stdin is `{"bookmarks":[]}`):** stderr silent. Stderr `Imported 0 bookmarks.` followed by newline. Exit 0. (Distinct from empty-stdin: the structural input is valid + naming-zero-imports is the legitimate empty-import semantic.)
...
**Threat model addition for stdin-fed attacker input.** `bm import` is the project's first surface to consume untrusted JSON. Existing defenses apply: serde_json default 128-level recursion limit defends against JSON depth-bomb (per [Security Review 1 Finding 6](vsdd-suite/review-log/2026-05-20-security.md) Hallucinated disposition); display_safe applies at the rendering boundary if imported bookmarks are subsequently rendered via `bm list`. New defenses warranted by stdin attack surface: **(a)** input-size cap (operator-decision: what's the maximum stdin byte count? 10MB? 100MB? unbounded with accepted-risk per the existing unbounded-URL framing?); **(b)** import-from-attacker-controlled-stdin per the Layer 1 threat-model framing on adversary-supplied URL contents — same disposition: store the bytes as-given; defer rendering safety to the `display_safe` discipline at output time. **AI-author note for operator:** the input-size cap is the key new threat-model decision for Layer 3; recommend defaulting to 10MB (matches the project's existing scale ceiling of 10,000 bookmarks at ~1KB each) with `--max-stdin-bytes <N>` flag for operator override + accepted-limitation framing for the cap choice.

**Why dedup-on-exact-tuple-match (the AI-author-default for `bm import` idempotence).** The conservative behavior prevents the common-mistake double-import (operator pipes `bm export | bm import` twice; without dedup the second import duplicates the records). The dedup is exact-tuple-match — url + timestamp + tags — not URL-match alone (URL-match-only dedup would conflict with the existing duplicate-URL multi-record semantic from Layer 2). **AI-author note for operator:** confirm exact-tuple-match is the right dedup granularity. Alternative: URL+timestamp match (ignores tags); URL-only match (most-aggressive dedup but conflicts with Layer 2 multi-record semantic); per-bookmark UUID injection at import time (most-precise dedup but requires schema addition).
...
**Layer 3 additions (AI-co-authored first-draft; operator edits + owns):**
...
- **`bm export` against a store with bookmarks containing control characters or newlines in URLs / tags:** `display_safe` wraps the offending string fields at serialization; emitted JSON remains valid + escape-clean. **AI-author note for operator:** confirm the `display_safe` placement leaves emitted JSON parseable by `bm import` (round-trip invariant).
...
- **`bm import` consuming a JSON file with bookmarks that have duplicate URLs within the imported payload itself:** all records are inserted as-given (the import does not de-duplicate WITHIN the input payload; it only de-duplicates against the existing destination state). This preserves the Layer 2 duplicate-URL multi-record semantic. **AI-author note for operator:** confirm this is the intended behavior, OR decide that within-payload duplicates should be dedup'd at import time.
- **`bm import` with stdin exceeding the input-size cap (10MB AI-author-default; operator may override via `--max-stdin-bytes <N>`):** error `Error: stdin exceeded maximum byte limit of <N>.` to stderr + exit 1. No partial import.
- **`bm import` after `bm export --tag <label>`:** the selective-copy semantic — only the tag-filtered subset is imported into the destination. **AI-author note for operator:** decide whether selective-copy is a documented use case OR an emergent behavior the spec stays silent on.
...
### Command surface (Layer 3 additions — AI-co-authored first-draft; operator edits + owns)
...
`bm export` emits the storage-format JSON to stdout; `bm import` reads the same shape from stdin. The pair enables backup / cross-machine sync / store-to-store migration via shell-pipe composition. **AI-author note for operator:** confirm the flag set above; `--max-stdin-bytes` is the AI-author-default for the input-size cap operator-override.
