...
- **Superseded by Layer 7 IAR Round 3 closure (commits `3fa1f3c` cmd_list extraction + column constants; `8db9437` `src/lib.rs` three-module split into `storage` / `validate` / `commands`).** Per CLOSURE-PROTOCOL §3 second paragraph, the auto-Backlog is explicitly reversible when the receiving authority later adjudicates — and the SA cluster was picked up and closed ~12h after the R2 Backlog entry was written. The "cost-benefit calculus … has not shifted enough" rationale above is no longer accurate as a forward-looking statement; the work was scheduled and shipped at Layer 7 R3. The Backlog → Resolved transition is the protocol working as designed; this annotation makes the supersedure explicit so the durable rationale on record matches the actual disposition. Same pattern as the SO Review 22 reversal annotations on SA Review 1 Finding 3 / SO Review 7. SO Review 25 Finding 1 (process bookkeeping correction).

---

## Layer 7 IAR Round 3 spec amendments

**`TRACKER_INTERNAL_FORCE_COLOR=1` test seam ratified as a documented (but unstable) spec element** — DESIGN.md "Interface / Color output" amended to name the environment variable explicitly: it bypasses TTY detection and overrides `NO_COLOR` / `CLICOLOR`, forcing `ColorMode::On`; not part of the public CLI contract; **unstable across versions**; intended solely to make the positive color contract automatable in integration tests (`assert_cmd::Command` pipes stdout, so the TTY-positive color path is otherwise reachable only through the manual TTY checklist).
- SO Review 25 Finding 2 (raised by SO; self-adjudicated by SO per CLOSURE-PROTOCOL §1 authority). Originating engineering need: QE Review 17 Finding 1 (closed by commit `bd7511e`).
- Why: an environment variable that demonstrably mutates observable color emission is, in the spec-fidelity sense, observable CLI behavior. DESIGN.md L243-244 promises "color is never emitted to a non-TTY stdout regardless of env vars, to preserve the pipe-cleanness contract"; the test seam violates that promise (deliberately, to enable automated coverage of the TTY-positive path). The pre-amendment state — variable read by the binary, never named in any `.md` file — left a contract gap: a reader auditing whether the implementation honors "regardless of env vars" by reading the spec alone would conclude yes and be wrong. Amending DESIGN.md to name the variable as a test-only and unstable seam (a) closes the contract gap, (b) preserves the engineering decision (the seam stays; the QE R17 F1 coverage gap remains closed by the 8 new integration tests in `tests/layer7.rs:280-502`), and (c) sets the precedent for any future test seam: name it in the spec or remove it. The "test-only by undocumentation" framing (hidden by being absent from `--help` / README / DESIGN.md but observable to anyone reading source) is not a defensible spec posture for a portfolio submission whose audience reads DESIGN.md as the authoritative contract.
- Trade-off accepted: the seam-before-NO_COLOR ordering means an end user who somehow learns the variable's name and sets it to `1` will override their own `NO_COLOR` accessibility preference. This is a test-ergonomics choice (CI environments that inherit `NO_COLOR=1` would otherwise require the test runner to `env_remove` it on every test invocation, which the 8 new force-color tests in `tests/layer7.rs` *do* explicitly). The user-safety cost is bounded by the variable's deliberately ugly, namespaced name (`TRACKER_INTERNAL_FORCE_COLOR` — the `INTERNAL_` segment is the "do not use" signal) and by the variable's unstable-across-versions disclaimer in DESIGN.md. Revisit if a future user reports unexpected color emission they cannot disable; in that case the seam should be reworked (e.g., compile-time `#[cfg(test)]` gating, or a different env-var prefix that is mechanically excluded from production builds).

---

## Layer 7 IAR Round 3 — non-blocking Open dispositions

**Test placement: 93 unit tests remain in `lib.rs#tests` rather than per-module `#[cfg(test)] mod tests` blocks** — SA Review 17 Finding 3 / QE Review 19 Finding 3 (raised; SO-adjudicated Backlogged per CLOSURE-PROTOCOL §3 auto-Backlog rule).
- Why: the rust supplement's per-module-tests criterion is real (colocated tests improve cohesion + clarify what each module owns), but the move requires distributing 93 tests across three modules AND solving the shared-test-helper problem (`issue`, `issue_with`, `issue_with_full`, `tracker_with` are used across what would become three different `#[cfg(test)] mod tests` blocks — they'd need to be either duplicated, moved to a `pub(crate) mod test_helpers`, or stay at lib.rs with cross-module visibility).
- The current arrangement works: 93 tests cohabit at `lib.rs#tests` with `use crate::commands::*; use crate::storage::*; use crate::validate::*;` glob imports, reaching every `pub(crate)` item across the three modules. Tests pass; mutation coverage is real; the test-target relationship is clear from each test's name (`title_*` tests live near each other; `tracker_validation_*` near each other; etc.).
- Trade-off accepted: cohesion gain at this project's scale (one developer, one codebase, ~1100 source LOC + ~1100 test LOC) is modest. The per-module move is the standard idiom for libraries published as crates; this project is `publish = false` and the lib is internal-only. Re-evaluate if the project's tests grow past ~300 (current 238) or if the lib is ever published.
- Backlog status (per CLOSURE-PROTOCOL §3): SA F3 / QE F3 carry the trigger "re-evaluate if test count exceeds 300 OR if the lib is ever published from this repo." Neither condition currently holds.

**Source-LOC coverage threshold trigger** — Platform Engineer Review 14 Finding 1 (informational re-raise from PE Review 8 Finding 3 / Round-1 closure escalation).
- Source LOC at HEAD: `src/lib.rs` ~80 (hub) + `src/storage.rs` ~218 + `src/validate.rs` ~273 + `src/commands.rs` ~700 = ~1271 non-test lines across the four files. The PE R8 F3 informal threshold of "~1000 source LOC" is now crossed.
- The threshold was originally proposed as the trigger for adding a coverage tooling gate (`cargo-llvm-cov` or `cargo-tarpaulin` in CI). PE Review 8 Finding 3 was Backlogged by SO Review 14 with the rationale "single-developer portfolio CLI; mutation coverage via clippy + cargo-fmt + the audit gate + the explicit test count is proportionate to scale." The threshold-crossing here is informational, not a re-raise of the dismissal.
- **SO ratification (this entry):** the dismissal stands. Mutation coverage at HEAD: 238 tests across 7 integration test files + 1 unit test module, with explicit per-helper exact-byte assertions (e.g., the new `force_color_data_row_emits_columns_in_status_then_priority_order` test pins column order). Coverage tooling would surface line-percentage metrics but does not add new defect-detection capability beyond what the existing tests + clippy + the `wrap_color` / `render_cell` `debug_assert!` patterns provide. Adding a coverage gate now would be ceremony, not protection.
- Reactivation trigger (per CLOSURE-PROTOCOL §3 mechanism): a real coverage-related defect missed by the existing test suite, or promotion of the project from `publish = false` to a published crate.

**Branch-protection / required-status-check verification** — Platform Engineer Review 12 Finding 1 / Platform Engineer Review 14 Finding 2 (informational, out of working tree).
- The `.github/workflows/` files in the repository declare a CI workflow with six gates (build / test / clippy / fmt / audit / deny). Whether those gates are *required-to-merge* on `main` is a GitHub repository-settings configuration, not a working-tree artifact. PE cannot verify the configuration from the working tree alone.
- **SO ratification (this entry):** the repository's branch-protection state is the director's responsibility to verify against the workflow. The director affirms (out-of-band): the CI workflow gates are wired correctly; required-status-check enforcement on `main` is a director-side configuration step that lives outside this codebase's verification surface. This finding is NOT a defect in this project's working tree — it is a process-pattern observation that future projects may benefit from a `CONTRIBUTING.md`-style checklist item ("after first push to a new repo, verify branch-protection settings match the workflow gate set"). VDD-IAR R14 Coordination raised this same pattern for suite-level consideration; no project-scoped action required.
