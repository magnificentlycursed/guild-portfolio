...
`CLICOLOR_FORCE=1` is **not** honored: color is never emitted to a non-TTY stdout regardless of env vars, to preserve the pipe-cleanness contract. Color suppression is symmetric: ANSI escapes are never emitted to stderr (the empty-state messages and error messages go to stderr per the stderr contract; they remain uncolored regardless of TTY state to keep error streams reliably scriptable).
