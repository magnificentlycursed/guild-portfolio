...
This directory began as the IAR adversarial review suite — domain prompts, dimensions, and a gap registry for VSDD Phase 3. It has since grown to house session primers for adjacent VSDD phases as well, making it a holistic VSDD prompt and process library rather than a pure Phase 3 review suite. The directory name and "IAR" identity are retained for continuity with prior project logs and external references; the artifact set is broader than the name implies.
...
- **Phase session primers** (`prompts/`) — posture-setting prompts for VSDD Phases 1, 1b, 2a–2b, and 3, plus a meta-primer for suite-development sessions
- **Language and interface supplements** (`supplements/`) — language- and interface-specific dimensions composed with domain reviews
- **Suite governance** — `SUITE-REVIEW-INDEX.md` (review index), `GAP-ANALYSIS-LOG.md` (gap registry), `CHANGELOG.md`, and `review-log/` session entries for the suite itself

Known scope gaps: VSDD Phase 4 (Feedback Routing) has no session primer ([G-86](GAP-ANALYSIS-LOG.md)). Phases 5 (Formal Hardening) and 6 (Four-Dimensional Convergence) are partially or wholly unowned (G-54, G-55).
...
|---|---|---|---|---|
| 1a | Spec Crystallization | Design doc written with behavioral contracts, edge case catalog, interface definitions, verification architecture | [`spec-crystallization.md`](prompts/spec-crystallization.md) | VDD-IAR Alignment dim 1 evaluates spec completeness |
| 1b | Decomposition | Project broken into layered TODO.md; Red Gate test plans written per layer; crosslink issue hierarchy created | [`decomposition.md`](prompts/decomposition.md) | VDD-IAR Alignment dims 2–3 evaluate layer structure and gate compliance |
| 2a | Red Gate | All tests written and failing before implementation begins | [`implementation.md`](prompts/implementation.md) (Phase 2a) | VDD-IAR Alignment dim 4 + QE dim 2 evaluate Red Gate compliance |
| 2b | Implementation | Tests made to pass; no new tests added during this phase | [`implementation.md`](prompts/implementation.md) (Phase 2b) | SE, QE, UX, Security domains evaluate implementation quality |
| **3** | **Adversarial Refinement** | **IAR runs until MVR** | [`review-session.md`](prompts/review-session.md) | **This is IAR** |
| 4 | Feedback Integration | Findings route back to the appropriate earlier phase: spec issues to Phase 1a, test issues to Phase 2a, implementation issues to Phase 2b | — (G-86) | IAR findings drive this loop; round count and finding progression are logged per domain |
| 5 | Formal Hardening | Proof harnesses, fuzzing, mutation testing (not yet owned by this suite — see GAP-ANALYSIS-LOG G-55) | — (G-55) | — |
...
The suite's own adversarial review history is indexed in [`SUITE-REVIEW-INDEX.md`](SUITE-REVIEW-INDEX.md), with individual session entries in `review-log/YYYY-MM-DD-suite-review.md`. Suite reviews are a single artifact type — sessions vary in mode (defect-search lens vs. registry-walk lens), and the mode lives in each entry's Lens field. See [`prompts/suite-development.md`](prompts/suite-development.md) for the entry format.
...
The `supplements/` folder, `GAP-ANALYSIS-LOG.md`, `SUITE-REVIEW-INDEX.md`, `review-log/`, and `prompts/` live in the suite template, not in individual projects.
