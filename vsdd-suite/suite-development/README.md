...
| **Effectful shell** — scripts that touch the filesystem, run subprocesses, or interact with external tools | `../hooks/check-anonymization.sh` (filesystem reads, grep, exit codes; repo-wide identity-revealing-pattern scan as of PR #43 — consolidates the prior `no-home-dir-paths` + `review-log-anonymization` hooks), `../hooks/check-external-review-anonymization.py` (identity-correlation discipline on external-review-log files), `../hooks/check-suite-internal-terminology.py` (user-facing-artifact containment of AI-agent vocabulary), `../templates/scaffold-project.sh` (filesystem writes, copies templates into target project) | Each effectful artifact has a documented purpose, an explicit no-op-on-empty behavior, and uses `set -euo pipefail` (or equivalent) to fail-fast on unexpected state. |
