...
- **Surface B (mutation testing).** The AI will rationalize surviving mutants as "equivalent" without proof. Each surviving mutant within the evaluation scope must be addressed: either (a) genuinely behavior-equivalent (named in writing with the proof of equivalence), (b) the test suite is missing a falsifying test (add it with the **retroactive-Red-Gate (Phase 5 source) label** per `primers/2b-implementation.md` — the same label discipline extends to post-MVR discovery), or (c) the spec has a gap the implementation correctly handled but the spec doesn't assert (route the surviving mutant to Phase 4 / Phase 1a+1b). A "this mutant is equivalent, trust me" line in the Phase 5 log is itself a finding for Phase 3's next round on the layer.
...
Before running any property-based tests, mutation tests, fuzzers, or proof harnesses, audit the implementation against `DESIGN.md` § Verification architecture's named purity boundary. The spec said which functions are pure (no I/O, deterministic, formally verifiable in principle); the implementation may have drifted. This audit is the prerequisite for every other Surface — Surface A asserts spec invariants over pure functions, but if the "pure" function quietly took on I/O during Phase 2b, the assertion is meaningless.

For each function on the verification-architecture purity-boundary list:
...
3. Record the audit outcome in the Phase 5 log preamble per layer: "Purity boundary verified for functions: <list>" OR "Boundary violations found and routed: <list with routing>".
...
1. Run the mutation tool. Standard tools:
   ...
   - **Python:** `mutmut`
2. Examine each surviving mutant. For each, choose one of the four outcomes named in the sycophancy check above. The Phase 5 log records the disposition per mutant.
