...
**Routing is not triage.** Triage prioritizes; routing relocates. Every real finding needs both: a priority (when to fix) and a phase (where to fix). A high-priority spec finding still routes to Phase 1a; a low-priority implementation finding still routes to Phase 2b. Priority does not change the phase a finding belongs to.
...
|---|---|---|---|
| Behavior is undefined in DESIGN.md and the implementation guessed | Spec gap | **Phase 1a** | Patch the spec; subsequent implementations inherit the contract |
...
5. **Close routed findings only when the routed work is gated.** A finding labelled `route:phase-1a` is *not* closed when the routing decision is made — it is closed when the spec revision passes its gate. Close with a comment that names the commit / DESIGN.md section that satisfies the route: `crosslink issue close <id> --comment "Routed to 1a; DESIGN.md §<X> revised in <commit>; self-adversary check passed in <session>."`
...
- **"Close on fix-applied"** — closing a routed finding when the *code* changes, not when the *gate* holds. A Phase 1a route is not done because someone edited DESIGN.md; it is done when the spec passes the self-adversary check.
...
The next pass begins. If the route included Phase 1a (spec revision), re-enter `primers/1a-spec-crystallization.md` for that section. If it included Phase 1b (re-decomposition), re-enter `primers/1b-decomposition.md` for the affected layers. If it included Phase 2a/2b, re-enter `primers/2b-implementation.md` for the routed work. The IAR refinement loop continues until MVR — see `README.md` § The refinement loop.
