...
The claude-code CLI supplement is the first per-tool supplement (registered at [Review 90](../suite-development/review-log/2026-05-23-suite-review.md#review-90--2026-05-23-1200z); the precedent for future per-tool supplements such as cursor.md / aider.md / codex-cli.md / chatgpt.md / anthropic-api-direct.md). It is an operational surface — the tool the methodology itself runs on. Per the [Three-audience design principle](../suite-development/suite-development.md#three-audience-design-principle-review-80-finding-3) ([Review 92 Finding 2](../suite-development/review-log/2026-05-24-suite-review.md#review-92--2026-05-24-0419z) cascade-applied for per-tool supplements):

- **Suite developers** (contributors extending claude-code-CLI-specific operational guidance) read this supplement to understand which CLI behaviors the methodology teaches as canonical + how to extend per-tool guidance when the CLI surface changes (new tool API; new plan tier; new model release; new Known Issues). Updates require verification against actual CLI behavior + cross-reference to [`claude-code-contract.md`](../claude-code-contract.md) for the dependency-contract framing (what the suite uses; what's needed but missing; upstream-coordination-ask candidates).
- **Suite users** (project teams running VSDD on the claude-code CLI) read this supplement to understand the operational discipline at the cycle layer — which plan tier they're on (per Plan tiers + rate-limit windows section); how prompt-cache discipline shapes their per-cycle cost (per the 5-minute TTL section); how Known Issues (parser-aborted heredoc; lettering-violation recurrence) apply to their authoring; how the cost-tally discipline frames the per-cycle observability surface. When running a multi-agent IAR cycle, the operator AND the AI agents reach for this supplement to calibrate the cycle's shape.
- **AI agents** (orchestrators running inside the claude-code CLI session) read this supplement as their own operational self-discipline surface — the tool they're running in is the tool this supplement governs. Per the Known Issues section, the agent must apply the workflow rules (Write / Edit over heredoc for >50-line content; cluster-spawn label discipline) to its own output. Per the Cost-tally discipline section, the agent applies the per-field auditability tier (per [Review 91 Finding 8](../suite-development/review-log/2026-05-23-suite-review.md#r91-f8)) to its own cost-tally authoring. Per the Wall-clock measurement pattern section, the agent invokes Bash `date -u` at session boundaries (per [Review 91 Finding 12](../suite-development/review-log/2026-05-23-suite-review.md#r91-f12)). The supplement is uniquely meta-applied: the agent reading it IS the agent the supplement describes.

The companion review dimensions per audience map: SO scopes which CLI behaviors are spec-promised by the suite-side dependency (suite-developer); Documentation Reviewer audits the supplement's clone-and-follow fidelity for a fresh operator (suite-user); AI Engineer audits the supplement's per-cycle discipline application — the meta-audit per [Review 91 Finding 14](../suite-development/review-log/2026-05-23-suite-review.md#r91-f14) cite-verify + [Finding 16](../suite-development/review-log/2026-05-23-suite-review.md#r91-f16) lookup-idiom (agent); TW audits the supplement's prose for cold-reader readability across the operational discipline (cross-audience).
