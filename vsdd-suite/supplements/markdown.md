...
These dimensions supplement the standard IAR domain reviews for markdown (`.md`) content authored inside VSDD projects and the [VSDD Suite](../README.md) itself. During each domain review of a markdown artifact — `DESIGN.md`, `TODO.md`, `PROCESS.md`, `CHANGELOG.md`, per-session review-log files, [`FINDINGS-INDEX.md`](../suite-development/FINDINGS-INDEX.md), optional per-domain index files (only present when the project opted in via the scaffold script's `--with-per-domain-indexes` flag — see [`suite-development/suite-development.md`](../suite-development/suite-development.md) § Structure), suite-development entries, [primers](../primers/), [domain prompts](../domains/), other [supplements](../supplements/), or any other `.md` artifact — apply the relevant section below in addition to the standard dimensions for that domain.
...
- **File-naming consistency** — Markdown file names use lowercase + hyphens (`anchor-link-convention.md`) except for canonical all-caps artifacts that the suite has standardized (`DESIGN.md`, `TODO.md`, `PROCESS.md`, `CHANGELOG.md`, `README.md`, `FINDINGS-INDEX.md`, and — when the project opts into the per-domain index — `<DOMAIN>-REVIEW.md`). The mix is intentional — caps signals "this is a load-bearing artifact in the methodology"; lowercase signals "this is content within a category."
...
- **The suite's existing hooks** — The suite ships seven hooks at [`vsdd-suite/hooks/`](../hooks/) that operate on text/markdown: [`check-anonymization.sh`](../hooks/check-anonymization.sh) (repo-wide identity-revealing-pattern scan; consolidates the prior `no-home-dir-paths` + `review-log-anonymization` hooks at PR #43), [`check-external-review-anonymization.py`](../hooks/check-external-review-anonymization.py) (external-review-log identity-correlation discipline; PR #42), [`check-suite-internal-terminology.py`](../hooks/check-suite-internal-terminology.py) (user-facing-artifact containment of suite-internal AI-agent vocabulary; PR #42), [`check-crosslink-references.py`](../hooks/check-crosslink-references.py), [`check-suite-review-preamble.py`](../hooks/check-suite-review-preamble.py), [`check-project-review-discipline.py`](../hooks/check-project-review-discipline.py), and [`check-changelog-currency.py`](../hooks/check-changelog-currency.py). New text/markdown hooks should follow the same patterns: pre-commit framework integration, per-file invocation, `# hook-bypass:` HTML-comment escape valve where applicable, registration in [`FINDINGS-INDEX.md`](../suite-development/FINDINGS-INDEX.md) when the hook closes a gap.
...
- **Be accurate when referring to people.** Operator handles use the canonical form (`dollspace.gay` per [github.com/dollspace-gay](https://github.com/dollspace-gay), not invented variants); anonymized text artifacts (review logs, CHANGELOGs, commit-message templates, any other committed text) use `<user>` / `<email>` / `<path>` placeholders per the repo-wide [anonymization hook](../hooks/check-anonymization.sh) (PR #43 — consolidates the prior `no-home-dir-paths` + `review-log-anonymization` hooks and applies to every committed text file, not just review-log markdown). External-review-author identities follow the additional [identity-correlation discipline](../suite-development/suite-development.md) enforced by [`check-external-review-anonymization.py`](../hooks/check-external-review-anonymization.py).
