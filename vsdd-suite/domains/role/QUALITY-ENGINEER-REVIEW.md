...
2. **Test falsifiability** — Would each test catch a broken implementation? Could any test pass against wrong code? A test that cannot fail on a defective implementation has no value.
   ...
   **Concrete mutation classes that recur (G-128, derived from ITC):** for every acceptance criterion, write a test that would fail if the criterion's boundary moves by one unit. Named mutation classes the project's tests must defend against: (a) **off-by-one in truncation/padding** — both directions: `49 chars + …` and `50 chars + …`, not just one (ITC L2 QE R7 caught one direction but not the symmetric one); (b) **sort-direction reversal** — `a.id.cmp(&b.id)` ↔ `b.id.cmp(&a.id)`; a test asserting "items are ordered" without asserting the specific direction passes both implementations (ITC L2 QE R7's sort-direction mutation survived 20 prior tests); (c) **inter-conjunct operator flips** — `&&` ↔ `||` in filter or predicate composition; each single-mismatch subcase short-circuits true on the matching conjunct under `||` (ITC L4 QE R11's compound-filter `&&` → `||` mutation survived all 5 unit tests because each test exercised only one filter dimension at a time); (d) **loop-exit polarity flip** — `while x { ... }` ↔ `while !x { ... }`; loops that exit on the first hit vs. last hit; (e) **comparison-operator polarity** — `>`, `>=`, `<`, `<=` all interchangeable for any single-point assertion, only the boundary test distinguishes them. **Assertion-style failure mode that compounds these:** substring `contains` assertions hide off-by-one and content-shape mutations — `assert!(output.contains("…"))` passes against `48 chars + …`, `49 chars + …`, `50 chars + …`, and `output is just …`; prefer **exact-match** assertions (`assert_eq!(output, "expected exact bytes")`) on every assertion whose expected output is invariant. Use `cargo mutants` (Rust) or equivalent to measure which mutations actually survive — a single survived mutation in production code is a coverage gap, not "good enough." Cross-reference Rust supplement § Quality Engineering for the `cargo mutants` invocation pattern.
