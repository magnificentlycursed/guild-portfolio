...
**Reviewer role: [Security](SECURITY-REVIEW.md) Engineer** (Security Engineer / Application Security Engineer)
...
**Coordination:** Flag any findings that should be surfaced to [QUALITY-ENGINEER-REVIEW.md](QUALITY-ENGINEER-REVIEW.md), [UX-REVIEW.md](UX-REVIEW.md), [PLATFORM-ENGINEER-REVIEW.md](PLATFORM-ENGINEER-REVIEW.md), [SOLUTION-ARCHITECT-REVIEW.md](SOLUTION-ARCHITECT-REVIEW.md), or [PRIVACY-REVIEW.md](PRIVACY-REVIEW.md) (dim 8 — data classification cross-references [Privacy](PRIVACY-REVIEW.md) dim 1; when Privacy is active, coordinate data classification findings there). For any sensitive data patterns identified in this review (secrets, PII, identity-leaking paths), flag them to [PLATFORM-ENGINEER-REVIEW.md](PLATFORM-ENGINEER-REVIEW.md) so they can be added to pre-commit hook detection rules.
...
Review entries are logged in per-session files at `vsdd-suite/review-log/YYYY-MM-DD-security.md` inside the project being reviewed; the project's `vsdd-suite/FINDINGS-INDEX.md` aggregates cross-cutting findings. The optional per-domain index at `vsdd-suite/SECURITY-REVIEW.md` activates when the project opts in via the scaffold script's `--with-per-domain-indexes` flag. See `vsdd-suite/suite-development/suite-development.md` § Governing standard for project-level review logs.
