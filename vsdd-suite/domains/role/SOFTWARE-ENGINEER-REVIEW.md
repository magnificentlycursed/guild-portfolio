...
**Reviewer role: [Software Engineer](SOFTWARE-ENGINEER-REVIEW.md)** (Software Engineer / Backend Engineer / Frontend Engineer)

The purpose of this review is to evaluate the quality of the implementation at the code level: correctness, clarity, error handling, naming, duplication, and complexity. Where the [Solution Architect](SOLUTION-ARCHITECT-REVIEW.md) review evaluates structure and boundaries, the Software Engineering review evaluates the code within those boundaries. Both matter. A well-structured module can still contain poorly written code.
...
12. **Test seam attack surface** — Does any code path constructed for test reachability ship in the release binary in a way that changes user-facing behavior or relaxes a documented invariant? Named failure modes: an env-var seam (`INTERNAL_FORCE_X=1` or similar) that bypasses a contracted behavior (color suppression, validation, authentication, rate limiting); `cfg(any(test, debug_assertions))` carve-outs whose `test`/`debug_assertions` half also compiles in release under common build configurations; `pub(crate)` visibility widening for test reachability that exposes internal contract surface to downstream callers; `debug_assert!` calls whose firing condition documents an invariant the production code does not actually enforce (`debug_assert!(value.is_ascii())` on a value the closed-enum validation should already enforce — if the assertion is needed, the enforcement is not real). Detector pattern: grep the codebase for `INTERNAL_`, `TEST_`, `_FORCE_`, `_BYPASS_`, `_SEAM`, `cfg(any(test`, `cfg(debug_assertions)`, `pub(crate)`, `debug_assert!` and verify each instance: (a) is the seam scoped to test-only compilation (`#[cfg(test)]`) or does it ship in release? (b) does the seam document an invariant that production code separately enforces? (c) could the seam, in release, be reached by user input or environment? Canonical worked example: ITC's `TRACKER_INTERNAL_FORCE_COLOR=1` env-var seam (`src/commands.rs:124`) shipped in release and mechanically bypassed DESIGN.md L244's "regardless of env vars" pipe-cleanness contract (caught at Round 3 by RT R12 F1; dismissed Low severity under single-user threat model — but the spec-violation was unambiguous and would NOT have been Low under a multi-user model). Resolution: either narrow the seam to `#[cfg(test)]`-only compilation (and adjust tests to set up state via the public interface rather than the seam), OR document the seam as an approved deviation in DECISIONS.md with explicit scope of when it MAY ship in release and what behavior it MAY override. Coordinate: [G-154](../../suite-development/FINDINGS-INDEX.md#g-154) cross-references SE (this dim), [Security](SECURITY-REVIEW.md) Dim 1 (input/env handling), [Red Team](RED-TEAM-REVIEW.md) (release-binary attack analysis); a finding here that's reproducible against a release build escalates to Security and Red Team via the standard coordination link.

---

### Extended: Documentation

These dimensions apply when the project has user-facing documentation or is evaluated as a portfolio or handoff artifact. For library projects, dim 16 (API and interface documentation) is always required. **When the [Technical Writer](TECHNICAL-WRITER-REVIEW.md) extended domain is active, defer documentation finding ownership to [TECHNICAL-WRITER-REVIEW.md](TECHNICAL-WRITER-REVIEW.md)** — apply these dimensions only when TW is not activated, as a floor. Log cross-domain findings in the TW log when both are active.

13. **README completeness** — Can someone new to the project understand what it does, how to install dependencies, how to run it, and how to run the tests from the README alone? Named checks: project purpose stated; prerequisites listed explicitly (runtime version, system dependencies); setup instructions that work from a clean checkout; test run command; known limitations or gotchas. The test: clone the repo into a fresh environment and follow the README. Any step that fails is a README failure.

14. **Documentation accuracy** — Does the documentation describe the current implementation? Named failure modes: README examples using a command that no longer exists; inline comments describing removed behavior; DESIGN.md features that were not implemented; function docstrings describing the previous signature. Stale documentation misleads — it is actively harmful, not merely incomplete.

15. **Decision rationale** — Are significant decisions recorded with their rationale, not just their outcome? DECISIONS.md entries that record "chose localStorage" without explaining why cannot be evaluated by a future developer. The rationale is what enables a future developer to judge whether the decision still applies.

16. **API and interface documentation** — Are public functions, types, and modules documented for callers? Named checks: exported functions have docstrings describing inputs, outputs, and error conditions; non-obvious fields are documented; the public interface surface is documented independently of the implementation. For library projects this is the primary deliverable alongside the code.
...
These dimensions apply when data volumes, runtime environment, or user expectations make performance a meaningful quality concern. For simple local tools with no network dependency and trivial data volumes, scope down significantly or skip. **When the [Performance Engineer](PERFORMANCE-ENGINEER-REVIEW.md) extended domain is active, defer performance finding ownership to [PERFORMANCE-ENGINEER-REVIEW.md](PERFORMANCE-ENGINEER-REVIEW.md)** — apply these dimensions only when PE is not activated. When both are active, log performance findings in the PE log to avoid duplicate finding ownership across logs.

18. **Main thread and event loop** — For browser apps and Node.js: are there synchronous operations that block user interaction for more than ~50ms? Named failure modes: synchronous sort or filter of large arrays; blocking JSON.parse on large payloads; synchronous localStorage reads on every keystroke; missing debounce or throttle on input handlers.
...
20. **N+1 access patterns** — Are there data access patterns that make one operation per item in a collection? Named failure modes: a storage read inside a loop; a derived value recomputed from storage on every render rather than cached; an event listener registered per item rather than using event delegation.

21. **Caching and memoization** — Are expensive computations reused across calls? Are derived values memoized when the underlying data has not changed? Is the cache invalidation strategy correct — does stale data get evicted appropriately? Does caching introduce correctness problems (stale state visible to the user)?

22. **Memory growth** — Does memory usage grow unboundedly over a user session? Named failure modes: event listeners added without removal (SA dim 5); objects accumulated in a closure that is never released; a cache with no eviction policy. This is a long-session failure — tests rarely run long enough to observe it.
...
Review entries are logged in per-session files at `vsdd-suite/review-log/YYYY-MM-DD-software-engineer.md` inside the project being reviewed; the project's `vsdd-suite/FINDINGS-INDEX.md` aggregates cross-cutting findings. The optional per-domain index at `vsdd-suite/SOFTWARE-ENGINEER-REVIEW.md` activates when the project opts in via the scaffold script's `--with-per-domain-indexes` flag. See `vsdd-suite/suite-development/suite-development.md` § Governing standard for project-level review logs.
