...
**Reviewer role: [Performance Engineer](PERFORMANCE-ENGINEER-REVIEW.md)** (Performance Engineer / Site Performance Engineer)
...
**Language and interface supplement:** Consult [`../../supplements/`](../../supplements/) for the supplement matching the project's primary language: [`rust.md`](../../supplements/rust.md), [`javascript-typescript.md`](../../supplements/javascript-typescript.md), [`python.md`](../../supplements/python.md). Apply the **Performance Engineer** section from the relevant supplement file in addition to the standard dimensions below — supplements specify language-specific tooling for profiling, benchmarking, and performance measurement.
...
Review entries are logged in per-session files at `vsdd-suite/review-log/YYYY-MM-DD-performance-engineer.md` inside the project being reviewed; the project's `vsdd-suite/FINDINGS-INDEX.md` aggregates cross-cutting findings. The optional per-domain index at `vsdd-suite/PERFORMANCE-ENGINEER-REVIEW.md` activates when the project opts in via the scaffold script's `--with-per-domain-indexes` flag. See `vsdd-suite/suite-development/suite-development.md` § Governing standard for project-level review logs.
