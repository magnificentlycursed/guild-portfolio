...
**Reviewer role: [Platform Engineer](PLATFORM-ENGINEER-REVIEW.md)** (Platform Engineer / DevOps Engineer / Infrastructure Engineer)
...
**Validator pair (Review 77):** `software-engineer` is the natural validator for PE findings whose fix is a CI / build-gate / tool-config change in the project tree. For PE shift-left mechanizations (new pre-commit hooks the suite authors to catch a recurring defect class), declare `**Validator:** sanity-check` per the meta-validator-of-last-resort pattern (Review 77 Finding 2) — [Sanity Check](../meta/SANITY-CHECK-REVIEW.md) confirms the hook's discipline-target coheres with DESIGN.md + the suite's architectural commitments. (Prior to Sanity Check's introduction, these landed as `*self*` with rationale; the 5 existing suite hooks predate the Sanity Check meta-domain and retain their original `*self*` framing per [G-89](../../suite-development/FINDINGS-INDEX.md#g-89) forward-only narrative-preservation.)
...
4. **Environment pinning** — Are runtime versions (Node, [Rust](https://www.rust-lang.org/) toolchain, [Python](https://www.python.org/), etc.) pinned to reproducible versions? Are tool versions for testing deterministic?
...
10. **[Pre-commit](https://pre-commit.com/) hooks** — Are pre-commit hooks installed and enforced to catch sensitive content before it enters version control? Hooks should cover: secret and credential detection (API keys, tokens, passwords, private keys, connection strings); PII detection (email addresses, phone numbers, government IDs, and other personal data); committer identity and local machine leakage (absolute paths containing usernames such as `/Users/yourname/`, hostnames, local environment details embedded in configs or build output); and large or binary files that should not be committed. Evaluate whether hooks can be bypassed with `--no-verify` and whether bypass is logged or blocked. Coordinate with [SECURITY-REVIEW.md](SECURITY-REVIEW.md) to ensure the detection patterns cover the project's full sensitive data surface.
11. **[Security](SECURITY-REVIEW.md) scanning** — Is a dependency audit run in CI and configured to fail on findings above the accepted risk threshold? (Use the ecosystem-appropriate tool: `npm audit`, `cargo audit`, `pip-audit`, `dependabot`, `trivy`, or equivalent.) See language supplement for specific tooling.
...
Review entries are logged in per-session files at `vsdd-suite/review-log/YYYY-MM-DD-platform-engineer.md` inside the project being reviewed; the project's `vsdd-suite/FINDINGS-INDEX.md` aggregates cross-cutting findings. The optional per-domain index at `vsdd-suite/PLATFORM-ENGINEER-REVIEW.md` activates when the project opts in via the scaffold script's `--with-per-domain-indexes` flag. See `vsdd-suite/suite-development/suite-development.md` § Governing standard for project-level review logs.
