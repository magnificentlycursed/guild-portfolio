...
10. **Methodology-canonical-defect deferral visibility** — When the project defers a defect whose fix IS the methodology's own canonical worked example (e.g., the VSDD whitepaper's atomic-write Adversary example in `01-how-we-build.md:137-139`), the DECISIONS.md entry for that deferral must explicitly name that fact alongside any engineering rationale. The defect remains contestable on engineering grounds (single-user, cost-disproportionate, etc.) and the deferral may still be the right call, but the entry must acknowledge "this defect is the methodology's own canonical worked example; a reviewer following the chapter will land here first" — to avoid the appearance of methodology cherry-picking. Engineering rationale alone is insufficient when the deferred defect is also the textbook example the chapter uses to teach the methodology being applied. Detector pattern: walk DESIGN.md and DECISIONS.md against the VSDD/VDD whitepaper sections cited (or that should be cited) in the project's methodology references; any defect appearing as both a project-level deferral and a chapter-level worked-example fix is a finding. Resolution: add a "Methodology-canonical-defect deferrals" annotation to the DECISIONS.md entry naming the chapter and section, preserving the engineering rationale, and explicitly framing the deferral as deliberate. Reference: G-153 in suite-development's [`FINDINGS-INDEX.md`](../../suite-development/FINDINGS-INDEX.md); the canonical worked example for this dim is ITC's non-atomic write deferral (SA Review 1 Finding 1, dismissed with single-user rationale) — which is dollspace.gay's own whitepaper's literal Adversary example.
