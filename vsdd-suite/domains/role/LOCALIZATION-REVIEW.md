...
Regression check: verify that locale handling established in prior layers has not been degraded by subsequent changes. A new UI component or format function added without i18n consideration is a regression even if prior components were correctly localized.
...
**Language and interface supplement:** Consult [`../../supplements/`](../../supplements/) for the supplement matching the project's primary language: [`rust.md`](../../supplements/rust.md), [`javascript-typescript.md`](../../supplements/javascript-typescript.md), [`python.md`](../../supplements/python.md). Apply the **Localization** section from the relevant supplement file in addition to the standard dimensions below — supplements specify language-specific i18n library guidance (Intl.* APIs, i18next, react-i18next for JavaScript/[TypeScript](https://www.typescriptlang.org/); fluent-rs and rust-i18n for [Rust](https://www.rust-lang.org/)).
