...
13. **Phase 5 discipline (G-55 / G-162 / G-177) — capstone + production intents** — Apply only when `DESIGN.md` § Project intent declares `**Phase 5 strategy:** planned — <scope>`. For projects whose Phase 5 strategy is `not applicable`, this dim records the declaration's presence as Demonstrated and skips substantive evaluation; for projects whose Phase 5 strategy is `planned`, evaluate:
    - **Surface activation matches strategy** — the Surfaces named in the strategy declaration each appear as a per-domain review log round with the `**Phase 5 surface:**` preamble tag (per G-177 closure): property-based testing / Purity Boundary Audit / Proof Execution in `vsdd-suite/SOLUTION-ARCHITECT-REVIEW.md` + linked `review-log/<date>-solution-architect.md`; Mutation Testing / Fuzz Testing in `vsdd-suite/QUALITY-ENGINEER-REVIEW.md` + linked `review-log/<date>-quality-engineer.md`. A strategy that names Surfaces A+B+C but rounds covering only B is a finding for under-delivery against the declared scope.
    - **Per-mutant disposition completeness** — Mutation Testing reports each surviving mutant's disposition (equivalent with named proof / missing-test-added / spec-gap-routed). Aggregate-only kill-rate reporting is a finding per `primers/5-formal-hardening.md` § Mutation Testing anti-pattern.
    - **Property-test invariant fidelity** — property-based testing properties express spec invariants from DESIGN.md, not generic liveness ("does not panic"). A property whose only assertion is "doesn't panic" is a finding per property-based testing anti-pattern.
    - **Fuzz-corpus persistence** — Fuzz Testing fuzz corpora are committed under `tests/` (or the language's idiomatic location); a layer that ran Fuzz Testing but didn't commit the corpus has lost the coverage-discovery audit trail.
