...
- **[`vsdd-suite/suite-development/review-log/2026-05-23-suite-review.md`](suite-development/review-log/2026-05-23-suite-review.md) Review 90** — audit-trail close documenting the 5-finding carryforward resolution with full per-finding Status/Owner/Validator/Evidence/Reasoning/Resolution-applied/Classification body per the canonical review-log discipline. SUITE-DEVELOPMENT-REVIEW Reviews-table row added above Review 88.
- **[`vsdd-suite/primers/3-review-session.md`](primers/3-review-session.md) § Pre-cycle methodology check** extended with a 10-field cost-tally report shape (Raw tokens canonical + Would-be API cost comparator-only + Actual cost to operator + Rate-limit-window utilization where observable + Wall-clock + Findings/100k tokens) + new required field "AI tool + plan tier + execution method" per Dim 14 + rate-limit-headroom field generalized from "Anthropic daily-token-limit" to "AI-tool/plan rate-limit-window" with per-tool supplement cross-references.
...
- **[`.pre-commit-config.yaml`](../.pre-commit-config.yaml)** — the previously-separate `no-home-dir-paths` hook (at `issue-tracker-cli/.pre-commit-hooks/check-no-home-paths.sh`; HOME-path-only, no allowlist) and `review-log-anonymization` hook (at `vsdd-suite/hooks/check-review-log-anonymization.sh`; HOME + name + email with allowlist, review-log scoped) are **consolidated into one suite-level repo-wide hook**. Per the operator's directive at PR #43: "Both anonymization hooks should be repo wide. Might make sense to combine them." The combined hook is a superset (HOME + git user.name + git user.email + public-URL allowlist) applied to every committed text file via `types: [text]`.
- **[`vsdd-suite/hooks/check-anonymization.sh`](hooks/check-anonymization.sh)** — renamed from `check-review-log-anonymization.sh` (via `git mv`) to reflect the broader scope. Docstring rewritten to name the consolidation. Public-URL allowlist extended with `bsky.app/profile/` per the queued operator-action item from PR #42. Dry-run against all currently-committed files: clean (zero pre-existing violations).
- **[`issue-tracker-cli/.pre-commit-hooks/check-no-home-paths.sh`](../issue-tracker-cli/.pre-commit-hooks/check-no-home-paths.sh)** — deprecation header added pointing at the consolidated hook. File preserved in place per G-89 forward-only narrative-preservation; the `.pre-commit-config.yaml` no longer references it.
- **[`vsdd-suite/suite-development/review-log/external-review-log/2026-05-21-shimmermathlabs.md`](suite-development/review-log/external-review-log/2026-05-21-shimmermathlabs.md)** — hook-bypass HTML comment removed (the bsky.app/profile/ allowlist addition makes it unnecessary); the PR-#43 self-reference HTML comment + the suite-internal-terminology-containment-hook "parallel to" reference both updated to name `check-anonymization.sh` (the post-consolidation filename).

### Changed (upstream-suite anonymization references — reflect consolidation in domains/primers/supplements)

Per the operator directive "Also update the anonymization dimensions in the upstream suite to reflect these changes" — every forward-facing methodology surface that referenced the old hook name(s) (`check-review-log-anonymization.sh`, `check-no-home-paths.sh`, `no-home-dir-paths`, `review-log-anonymization`) or the old narrow-scope language ("scans review-log markdown", "block local home directory paths pre-commit hook") now names the consolidated `check-anonymization.sh` hook and the repo-wide all-committed-text-files scope. Historical anchors (`FINDINGS-INDEX.md` G-98/G-122/G-129; `SUITE-DEVELOPMENT-REVIEW.md` Review 36/Review 76 row summaries; per-session review-log files for 2026-05-06/17/18/19/20/21) are preserved under [G-89](suite-development/FINDINGS-INDEX.md#g-89) forward-only narrative-preservation — they correctly name what existed at the time of the entry.

- **[`vsdd-suite/domains/role/SECURITY-REVIEW.md`](domains/role/SECURITY-REVIEW.md)** § Confidentiality-aware citation — Security-domain reminder names `check-anonymization.sh` and the consolidation context.
- **[`vsdd-suite/domains/role/PLATFORM-ENGINEER-REVIEW.md`](domains/role/PLATFORM-ENGINEER-REVIEW.md)** § Confidentiality-aware citation — Platform-domain reminder names `check-anonymization.sh` and the consolidation context.
- **[`vsdd-suite/domains/role/AI-ENGINEER-REVIEW.md`](domains/role/AI-ENGINEER-REVIEW.md)** § Error-event ownership boundary — early-detection-scripts/hooks list updated to name `check-anonymization.sh` + `check-external-review-anonymization.py` + `check-suite-internal-terminology.py` (was: `check-review-log-anonymization.sh`; the AI-Engineer-owned hook surface now reflects the PR #42 + PR #43 cumulative set) and fixes a stale `check-crosslink-references.sh` reference to the actual `.py` filename (post-Review-76 rename).
- **[`vsdd-suite/primers/3-review-session.md`](primers/3-review-session.md)** § Confidentiality-aware citation — both paragraphs updated: the "common signals that the project is opt-in anonymized" paragraph now names the repo-wide anonymization hook scope; the suite-level-controls paragraph names `check-anonymization.sh` + the consolidation history + the broadened scope + the companion `check-external-review-anonymization.py` hook.
- **[`vsdd-suite/primers/6-convergence.md`](primers/6-convergence.md)** § Phase 6 — Anonymization-aware attestation paragraph updated to name the repo-wide `anonymization` hook in the "scrub me" signal list.
- **[`vsdd-suite/suite-development/README.md`](suite-development/README.md)** § What lives here — Effectful-shell table row updated to name the post-consolidation hook + the two new PR-#42 hooks (`check-external-review-anonymization.py`, `check-suite-internal-terminology.py`).
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** § Identity-correlation discipline — the **Why:** paragraph now names the single repo-wide consolidated hook (was: two separate hooks) and explains the PR-#43 consolidation alongside the companion `check-external-review-anonymization.py` hook.
- **[`vsdd-suite/supplements/bash.md`](supplements/bash.md)** — § Authored paragraph names `hooks/check-anonymization.sh` (post-rename) and the broadening; PE § No-hardcoded-paths bullet names the repo-wide consolidated hook + drops the legacy `check-no-home-paths.sh` reference.
- **[`vsdd-suite/supplements/markdown.md`](supplements/markdown.md)** — "Suite's existing hooks" bullet now enumerates seven hooks (was: five) and names the consolidation; TW § "Be accurate when referring to people" updated to point at `check-anonymization.sh` + the broader anonymized-text-artifacts scope; both bullets cross-link to the identity-correlation discipline + `check-external-review-anonymization.py`.
- **[`vsdd-suite/supplements/github-actions.md`](supplements/github-actions.md)** § Error-event ownership table — Early-detection-scripts row updated to name the post-consolidation + PR-#42-new hook set (parallel to the AI-Engineer-domain ownership-boundary list above).
...
- **[`vsdd-suite/supplements/github-actions.md`](supplements/github-actions.md)** — new multi-domain CI/CD platform supplement (~280 lines) covering 8 role-domain perspectives: Platform Engineer (PRIMARY — workflow shape, job decomposition, runner selection, matrix builds, caching, concurrency, reusable workflows, MSRV+toolchain pinning consistency); Security (SHA-pinning third-party actions, `permissions:` block at most-restrictive granularity, secret handling, `pull_request_target` pwn-request audit, GITHUB_TOKEN scope minimization); AI Engineer (CI cost discipline at runner-minute scale — per-merge budget bands; cache hit rate as runner-minute multiplier; re-run strategy; matrix-shape token-economy analogue); Technical Writer (workflow YAML readability + named-step discipline); Documentation Reviewer (clone-and-follow fidelity via `act`; workflow file as cold-readable documentation); Solution Owner (spec-promised vs incidental CI behaviors; required-vs-optional checks in branch protection; PR template + merge-gate as SO-owned); Quality Engineer (Red Gate at CI; coverage thresholds); Performance Engineer (workflow execution-time budget; cache effectiveness). 11 anti-patterns + Three-audience lens application. Closes [Review 86 Finding 1](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).
- **[`.github/workflows/pr-checklist.yml`](../.github/workflows/pr-checklist.yml)** — new merge-gating workflow. Parses PR body on every `pull_request` event (opened / edited / synchronize / reopened / ready_for_review); identifies the `## Completion checklist (merge-gating)` section; fails the run if any `- [ ]` items remain unchecked. Branch-protection rule configuration is operator-action post-merge (configure "PR completion checklist / verify-checklist" as required status check via GitHub UI; AI cannot configure repo settings without authorized scope). Closes [Review 86 Finding 2](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).

### Changed (operational artifacts — PR template + workflow, [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z))

- **[`.github/PULL_REQUEST_TEMPLATE.md`](../.github/PULL_REQUEST_TEMPLATE.md)** — rewritten per the new GitHub Actions supplement § PR template + merge-gate integration: test plan REFERENCES `manual-tests/layer-N.md` + `manual-tests/install-verification.md` canonically (does NOT duplicate their content per [Documentation Reviewer Dim 4](domains/role/DOCUMENTATION-REVIEWER-REVIEW.md) cross-reference resolution discipline); structured merge-gating completion checklist sub-divided into Pre-commit + CI / Audit-trail discipline / Methodology discipline / PR draft + spot-checks / Operator-action queue; spot-check discipline named (verifiable conditions with evidence linked, NOT informal-prose checkboxes).
- **[`.github/workflows/bookmark-cli-manual.yml`](../.github/workflows/bookmark-cli-manual.yml)** — 3 baseline gaps closed against the new supplement: `permissions: contents: read` block added at workflow level (Security § `permissions:` block discipline); `concurrency: { group: 'bookmark-cli-manual-${{ github.ref }}', cancel-in-progress: true }` added (Platform Engineering § Concurrency control); `timeout-minutes: 10` per job (Platform Engineering § Workflow execution-time budget; 10 min cap catches hangs at ~2x observed median). Closes [Review 86 Finding 3](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).

### Changed (reference example — PROCESS.md three-audience-lens optimization, [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z) Finding 4)

- **[`vsdd-suite-reference-examples/bookmark-cli-manual/PROCESS.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/PROCESS.md)** — AI-author "what was hardest" / "what I got wrong" / "what the process felt like" claims evaluated against review-log evidence — **all 3 PROVEN OUT** (F-004 Purity Boundary Audit divergence; F-001 Phase 2a→2b commit boundary; F-005 Mutation Testing kill rate); existing 3 stumbling points gained `**Three-audience lens:**` paragraphs naming what suite developers / suite users / AI agents should each take from the stumbling point. **3 NEW post-PR-#38/#39/#40 stumbling points added** with three-audience treatment + review-log citations: (4) 80 Round-1 findings as spec/test under-investment signal disguised as IAR thoroughness; (5) operator-directive correction cost — 3 mid-cycle slips in PR #38 (Round 2 filename violation; wrong adversarial-pair clustering; cluster-letter naming); (6) site-specific fix declared closure — Doc Reviewer R3 carryforward pattern that motivated the `grep -rn before claiming closure` discipline. Top-of-file note explains the three-audience-lens optimization per [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).

### Resolved (added to existing Reviews 84 + 85 closures)

- **[Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z) Finding 1** — GitHub Actions supplement authored. Full narrative: [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).
- **[Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z) Finding 2** — PR template rewrite + `pr-checklist.yml` merge-gating workflow. Full narrative: [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).
- **[Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z) Finding 3** — bookmark-cli-manual.yml workflow update against new supplement. Full narrative: [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).
- **[Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z) Finding 4** — PROCESS.md three-audience-lens optimization + 3 AI-author claims PROVEN OUT against review-log + 3 NEW stumbling points added. Full narrative: [Review 86](suite-development/review-log/2026-05-21-suite-review.md#review-86--2026-05-21-1200z).

**Backlog after Reviews 84 + 85 + 86: 1 Open ([Review 79 Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** (unchanged — Reviews 85 + 86 produced no new suite-side findings beyond in-session Resolved; operator-action queue extends with the new branch-protection-rule configuration ask alongside the 13 upstream crosslink filings).

### Added (suite-side — external-feedback mining, [Review 85](suite-development/review-log/2026-05-21-suite-review.md#review-85--2026-05-21-1130z) Finding 1)

- **External-feedback artifact recognized:** [`vsdd-suite/suite-development/review-log/2026-05-20-crosslink-value-add-review.txt`](suite-development/review-log/2026-05-20-crosslink-value-add-review.txt) — dollspace-gay's value-add review evaluating vsdd-suite-with-crosslink integration vs crosslink alone (source artifact dated before 11:32 AM Pacific 2026-05-20 ≈ ~18:32 UTC, co-temporal with [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z); mining deferred ~24 hours because non-standard `.txt` filename + structure didn't trigger the suite-review hook canonical-pattern). Mining outcome: **7 crosslink CLI bugs** identified for operator-to-file upstream (CLI papercuts discovered through vsdd-suite usage); **5 absorbability concepts** identified for upstream coordination (Phase primers as crosslink skills; 16-domain adversarial review prompt set; language × interface supplements composition; forward-only compatibility doctrine; cold-context dispatch naming); **2 validation findings** ([`hooks/check-crosslink-references.sh`](hooks/check-crosslink-references.sh) design validated; reverse-signal disclaim list validated). **All routed to operator-action queue** for upstream crosslink filing; no suite-side fixes needed per [`suite-development.md`](suite-development/suite-development.md) § External dependency references discipline. **The vsdd-suite design is validated by the methodology author** (dollspace-gay = VSDD whitepaper + crosslink author).

### Added (suite-side — upstream methodology fixes, [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 1)

- **[`vsdd-suite/primers/3-review-session.md`](primers/3-review-session.md)** § Pre-cycle methodology check — new section before "Before starting a domain review." Declares the discipline that multi-agent IAR cycles (parallel cold-session at capstone or production intent; cluster-batching cycles; cycles with >4 agent-spawns) open with a pre-cycle declaration naming spawn shape + per-cycle budget + rate-limit headroom + model selection per task class. Closes [AI Engineer R1 F8](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md) Dim 13.
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** § Per-review entry preamble § Cost-tally — new optional preamble field for capstone + production intent multi-agent cycle-closing entries. Total agent-spawns, estimated total token consumption, per-substantive-finding token cost, rate-limit-hit events, model-selection actual-vs-declared. Closes [AI Engineer R1 F6](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md) Dim 2.
- **[`vsdd-suite/domains/DOMAIN-INDEX.md`](domains/DOMAIN-INDEX.md)** § Cold-session budget per intent — new subsection under Intent calibration with per-intent bands (learning-exercise / portfolio / capstone / production each with max rounds + max agents per round + per-finding token cost band + model selection ceiling). First-pass estimates derived from bookmark-cli-manual PR #38 + AI Engineer R1 cost evidence. Closes [AI Engineer R1 F7](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md) Dim 9.
- **[`vsdd-suite/templates/DESIGN-template.md`](templates/DESIGN-template.md)** § Cold-session budget — new declaration line under Phase 5 / Phase 6 strategy block; required at capstone + production intent. Closes AI Engineer R1 F7 template-application surface.
- **[`vsdd-suite/primers/4-feedback-integration.md`](primers/4-feedback-integration.md)** § Anti-patterns — new "Site-specific fix declared closure" anti-pattern. Every Resolved finding for a defect class (stale path reference, retired terminology, broken anchor link, duplicate-name sweep artifact, letter-coded identifier residue) requires audit-trail evidence that a project-wide grep returns clean for the defect class. Closes the [Doc Reviewer R3 carryforward pattern](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-20-documentation-reviewer.md) root cause.

### Changed (suite-side — per-domain index made optional, [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 2)

- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** § Structure — heading retitled `(per-domain index + per-session entries)` → `(per-session entries + cross-cutting registry; per-domain index is optional)`. Canonical project-level review-log structure becomes `review-log/` + `FINDINGS-INDEX.md`; the per-domain index file (`vsdd-suite/<DOMAIN>-REVIEW.md`) is optional, activated by the project when it wants a domain-organized navigation surface.
- **[`vsdd-suite/templates/scaffold-project.sh`](templates/scaffold-project.sh)** — new `--with-per-domain-indexes` opt-in flag (default off). The per-domain index files are no longer created by default; opt-in is by passing the flag.
- **[`vsdd-suite/templates/DOMAIN-REVIEW-template.md`](templates/DOMAIN-REVIEW-template.md)** — HTML-comment preamble note at the top marking the template OPTIONAL as of Review 84. The template stays for projects that opt in.
- **[`vsdd-suite/templates/README.md`](templates/README.md)** — DOMAIN-REVIEW-template row reframed; Customization checklist conditional on opt-in.
- **[`vsdd-suite/README.md`](README.md)** — Quickstart steps + worked-example mentions updated to clarify the per-domain index layer is optional. Primary navigation surface is `review-log/` + `FINDINGS-INDEX.md`.
- **[`vsdd-suite/COMPATIBILITY.md`](COMPATIBILITY.md)** — new v0.13.0 row naming the per-domain-index optional shift.
- **All 19 domain prompt files at [`vsdd-suite/domains/role/*-REVIEW.md`](domains/role/) + [`vsdd-suite/domains/meta/*-REVIEW.md`](domains/meta/)** — closing line reworded to name `review-log/` + `FINDINGS-INDEX.md` as canonical navigation, with the optional per-domain index activated via the scaffold script's opt-in flag.
- **[`vsdd-suite/supplements/markdown.md`](supplements/markdown.md)** — opening artifact list reframed; canonical-all-caps list line updated.

### Changed (reference example — bookmark-cli-manual, [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 2)

- **13 per-domain index files deleted** at [`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/<DOMAIN>-REVIEW.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/) (12 role + 1 meta — SE / QE / UX / Security / SA / SO / VDD-IAR Alignment / Performance Engineer / Platform Engineer / Red Team / TW / Documentation Reviewer / AI Engineer). The reference example now navigates exclusively via `review-log/` + `FINDINGS-INDEX.md`.
- **~5 forward-facing project files updated** with reference rewrites: DESIGN.md (3 refs across 2 lines), README.md (1 nav sentence), CHANGELOG.md (1 ref in v0.12.0 entry), src/lib.rs (1 doc-comment cite), manual-tests/install-verification.md (1 ref to PE index). Forward-only carve-out per [G-89](suite-development/FINDINGS-INDEX.md#g-89): CHANGELOG entries v0.11.5 and earlier + all 14 per-session review-log files left untouched as historical record.
- **[`vsdd-suite-reference-examples/bookmark-cli-manual/DESIGN.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/DESIGN.md)** § Project intent — new Cold-session budget declaration line under the Phase 6 strategy declaration. References the actual cost evidence from PR #38 + PR #39 cycles (~$5/cluster Round 3 cost; ~21k tokens/finding AI Engineer R1 — well below the capstone-intent band's floor, read as cold-session discipline working efficiently).

### Changed (suite-side — Three-audience design principle rename + lens-application sweep, [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 4)

- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** [§ Three-audience design principle](suite-development/suite-development.md#three-audience-design-principle-review-80-finding-3) — heading renamed from "Dual-audience design principle" to "Three-audience design principle" (the body of the [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 3 sub-section already named three audiences — heading lagged the prose). Old HTML anchor `dual-audience-design-principle-review-80-finding-3` preserved at the section per [G-89](suite-development/FINDINGS-INDEX.md#g-89) for backward link compatibility; new `three-audience-design-principle-review-80-finding-3` anchor for forward-facing references. Companion-review-dimensions-per-audience table authored (SO = suite-developer audience; Doc Reviewer = suite-user audience; AI Engineer = AI-agent audience; TW = cross-audience narrative).
- **9 primers** (`vsdd-suite/primers/1ab-spec-crystallization.md` through `6-convergence.md`) — each gained a closing `## Three-audience lens` section naming how each audience uses the primer.
- **19 domain prompt files** (`vsdd-suite/domains/role/*-REVIEW.md` + `vsdd-suite/domains/meta/*-REVIEW.md`) — 4 primary-audience domains ([AI Engineer](domains/role/AI-ENGINEER-REVIEW.md) / [Documentation Reviewer](domains/role/DOCUMENTATION-REVIEWER-REVIEW.md) / [Technical Writer](domains/role/TECHNICAL-WRITER-REVIEW.md) / [Solution Owner](domains/role/SOLUTION-OWNER-REVIEW.md)) gained a `## Three-audience lens for this domain` section; 15 cross-audience domains gained a single closing-line cross-reference naming the principle.
- **[`vsdd-suite/suite-development/SUITE-DEVELOPMENT-REVIEW.md`](suite-development/SUITE-DEVELOPMENT-REVIEW.md)** preamble — three-audience paragraph cross-referencing the principle + Agent-API surface.
- **[`vsdd-suite/templates/PROJECT-FINDINGS-INDEX-template.md`](templates/PROJECT-FINDINGS-INDEX-template.md)** + **[`vsdd-suite/templates/PROJECT-README-template.md`](templates/PROJECT-README-template.md)** — three-audience cross-references mirroring the suite-side artifacts.
- **[`vsdd-suite/README.md`](README.md)** § Suite scope — three-audience paragraph cross-referencing the principle + Agent-API surface.
- **[`vsdd-suite/CHANGELOG.md`](CHANGELOG.md)** line 134 (Review 80 entry's `Added` block) — "Dual-audience" → "Three-audience" rewrite; anchor target updated to the new anchor.

### Changed (reference example — bookmark-cli-manual, [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 4)

- **[`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/FINDINGS-INDEX.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/FINDINGS-INDEX.md)** preamble — three-audience paragraph cross-referencing the principle + Agent-API surface.
- **[`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md)** Finding 3 — header + body + Validator rationale rewritten with "dual-audience" → "three-audience" (3 sites; inline `<!-- [Review 84 rename] -->` notes). The file is still in active discipline-iteration (PR #40 is the same cycle as PR #39); forward-facing rewrite is allowed.

### Resolved

- **[Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 1** — 4 upstream methodology fixes closing AI Engineer R1 F6/F7/F8 + Doc Reviewer carryforward pattern. Full narrative: [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z).
- **[Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 2** — Per-domain index files retired at the project level; suite-side template remains as opt-in stub via new `--with-per-domain-indexes` scaffold flag; 13 files deleted at bookmark-cli-manual; suite-side cascade across 19+ files. Full narrative: [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z).
- **[Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 3** — Documentation Reviewer Round 4 verification closes 5 R3 carryforwards via per-domain-index retirement (3) + grep-clean evidence (2). Doc Reviewer MVR reached. bookmark-cli-manual project MVR scorecard promotes from 7 of 10 → 9 of 10 at MVR. Phase 6 four-dimensional convergence remains DEFERRED (Platform Engineer install-verification operator-gate). Full narrative: [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z).
- **[Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 4** — "Dual-audience design principle" renamed to "Three-audience design principle"; companion-review-dimensions-per-audience table authored; three-audience lens applied across 9 primers + 19 domain prompts + 2 indexes + 2 templates + suite README + AI Engineer R1 cold-session log rewrite. Old `dual-audience-design-principle-review-80-finding-3` HTML anchor preserved per G-89 for backward link compatibility. Full narrative: [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z).
- **[Review 85](suite-development/review-log/2026-05-21-suite-review.md#review-85--2026-05-21-1130z) Finding 1** — External-feedback mining of dollspace-gay value-add review (mining deferred ~24 hours; operator-surfaced during PR #40). 7 crosslink CLI bugs + 5 absorbability concepts + 2 validation findings; all routed to operator-action queue for upstream crosslink filing; no suite-side fixes. The vsdd-suite design is validated by the methodology author. Full narrative: [Review 85](suite-development/review-log/2026-05-21-suite-review.md#review-85--2026-05-21-1130z).

**Backlog after Reviews 84 + 85: 1 Open ([Review 79 Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** (the AI Engineer R1 F6 / F7 / F8 — previously Deferred-pending-this-PR — are now Resolved via Review 84 Finding 1's methodology fixes; Review 85's external-feedback mining produced no new suite-side findings — all observations routed to operator-action queue for upstream crosslink filing).

---

## v0.12.0 — 2026-05-21 10:00Z ([Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z): AI Engineer role-domain registered + first cold-session round on bookmark-cli-manual — 2 Findings Resolved; v0.12.0; PR [#39](https://github.com/magnificentlycursed/guild-portfolio/pull/39))

### Added (suite-side — new role-domain registration, [Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z) Finding 1)

- **[`vsdd-suite/domains/role/AI-ENGINEER-REVIEW.md`](domains/role/AI-ENGINEER-REVIEW.md)** — new extended role-domain authored. The AI Engineer is the cost-and-quality discipline applied to how AI agents are used to drive the IAR cycle: 13 evaluation dimensions covering session isolation, token economy per finding (with intent-tier expected bands), Anthropic prompt-cache discipline, sub-agent delegation patterns, rate-limit strategy + graceful degradation, model selection per task class (Opus / Sonnet / Haiku), cluster-batching with adversarial-pair separation, Phase 4 routing as Round-2+ scope-reducer, cold-session-budget declaration per intent tier, memory-leakage between sessions, audit-trail machine-readability cost, operator-directive correction cost, pre-cycle methodology check. **Classification universe:** Resolved / Deferred / Dismissed / Hallucinated (same as Software Engineer / Documentation Reviewer / Performance Engineer / Platform Engineer). **Validator:** `sanity-check` (no canonical role-domain pair — Dim concerns span the whole methodology). **Activation:** capstone + production active by default; portfolio active when project uses parallel cold-session adversarial review; extended-pool opt-in at learning-exercise.
- **[`vsdd-suite/suite-development/review-log/2026-05-21-suite-review.md`](suite-development/review-log/2026-05-21-suite-review.md)** — new dated suite-review file (Review 83 is the first review on 2026-05-21; per the suite-side per-date convention, gets its own file).

### Changed (suite-side cascade for AI Engineer registration)

- **[`vsdd-suite/domains/DOMAIN-INDEX.md`](domains/DOMAIN-INDEX.md)** — AI Engineer added to the Extended role-domains table; intent-calibration pool listing extended (`{PE, DE, Red Team, Performance Engineer, Technical Writer, Documentation Reviewer, AI Engineer, Accessibility, Privacy, Localization}`).
- **[`vsdd-suite/hooks/check-project-review-discipline.py`](hooks/check-project-review-discipline.py)** `DOMAIN_CLASSIFICATIONS` — `ai-engineer` registered with the standard role-domain classification universe (`{"Resolved", "Deferred", "Dismissed", "Hallucinated"}`).
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** § Finding classification schemas by domain type — AI Engineer added to "most role domains" classification row; canonical domain-slug list extended with `ai-engineer`.
- **[`vsdd-suite/README.md`](README.md)** § Extended role domains catalog — new AI Engineer row.
- **[`vsdd-suite/suite-development/SUITE-DEVELOPMENT-REVIEW.md`](suite-development/SUITE-DEVELOPMENT-REVIEW.md)** — Review 83 row added at the top of the Reviews table.
- **[`vsdd-suite/suite-development/FINDINGS-INDEX.md`](suite-development/FINDINGS-INDEX.md)** — new r83-f1 + r83-f2 rows.

### Added (reference example — bookmark-cli-manual, [Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z) Finding 2)

- **[`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/AI-ENGINEER-REVIEW.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/AI-ENGINEER-REVIEW.md)** — new per-domain index stub at the reference example (capstone-active-domain count promoted from 11 → 12 role-domains; 13 total active).
- **[`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-21-ai-engineer.md)** — Round 1 cold-session review against PR [#38](https://github.com/magnificentlycursed/guild-portfolio/pull/38)'s 3-round cycle. **10 findings filed: 5 Resolved + 3 Deferred + 1 Dismissed + 1 Hallucinated**. Resolved disciplines (codified for regression-check): cluster-batching with adversarial-pair separation (Dim 7); Phase 4 routing as Round-2+ scope-reducer (Dim 8); audit-trail machine-readability passes [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 3 Agent-API contract (Dim 11); operator-directive correction cost — 3 mid-cycle slips codified back into the methodology (Dim 12); rate-limit graceful-degradation discipline vindicated (Dim 5). Deferred to PR [#40](https://github.com/magnificentlycursed/guild-portfolio/pull/40) upstream-suite-remediation: token-tally discipline missing from audit trail (Dim 2); cold-session-budget declaration absent from DESIGN.md template (Dim 9); pre-cycle methodology check missing from primer 3 (Dim 13). MVR NOT reached at Round 1 per G-131 continue-trigger. **Per-finding token cost: ~21k/finding** — well below the capstone-intent floor of 100k/finding, read as cold-session discipline working well.

### Changed (reference example — active-domain count updated)

- **[`vsdd-suite-reference-examples/bookmark-cli-manual/DESIGN.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/DESIGN.md)** § Project intent — Active domain set: `5 extended` → `6 extended`; `11 role + 1 meta = 12 active domains` → `12 role + 1 meta = 13 active domains` (AI Engineer added).
- **[`vsdd-suite-reference-examples/bookmark-cli-manual/TODO.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/TODO.md)** — `12 active domains` → `13 active domains`; capstone-tier extended-set list extended with AI Engineer.
...
- **[Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z) Finding 1** — AI Engineer role-domain registered (cost-and-quality discipline for AI-agent usage in the IAR cycle); 13 dimensions; validator: sanity-check; capstone + production active by default; cascade across hooks + indexes + classification schema + reference example complete. Full narrative: [Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z).
- **[Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z) Finding 2** — AI Engineer Round 1 cold-session against bookmark-cli-manual filed (5 Resolved + 3 Deferred + 1 Dismissed + 1 Hallucinated); MVR NOT reached; Round 2 routes to PR [#40](https://github.com/magnificentlycursed/guild-portfolio/pull/40). Full narrative: [Review 83](suite-development/review-log/2026-05-21-suite-review.md#review-83--2026-05-21-1000z).

**Backlog after Review 83: 1 Open ([Review 79 Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** (the 3 newly-Deferred AI Engineer findings are project-scope, tracked in the [project FINDINGS-INDEX](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/FINDINGS-INDEX.md) — they route to PR #40 as upstream-suite-remediation methodology-authoring work; they don't roll up to the suite-side registry).

---

## v0.11.4 — 2026-05-20 20:00Z ([Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z): bookmark-cli-manual capstone 6-phase IAR Round 1 execution — 10 parallel cold-session rounds, 80 findings, Phase 6 deferred; methodology-vindication NUL-byte corruption fix from PR [#37](https://github.com/magnificentlycursed/guild-portfolio/pull/37) — 2 Resolved; v0.11.4; PR [#38](https://github.com/magnificentlycursed/guild-portfolio/pull/38))
...
- **[Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z) Finding 2** — bookmark-cli-manual capstone 6-phase IAR Round 1 execution (10 cold-session rounds; 80 findings). Full narrative: [Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z).
- **[Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z) Finding 3** — Round 1 fix cycle executed in 5 batches (spec / code+tests / docs / config / CI) per operator directive ("ship the layer correctly"); 30+ files modified across `vsdd-suite-reference-examples/bookmark-cli-manual/`; layer's cargo test/clippy/fmt clean (13 integration + 8 unit tests pass). Full narrative: [Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z).
- **[Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z) Finding 4** — Round 2 cold-session verification (10 parallel agents); only VDD-IAR Alignment at MVR (with operator-block carve-out); Platform Engineer MVR-blocked-by-operator-gate (Dim 38 install-verification requires non-author on fresh system); 8 other domains have 14 new Round 2 findings + 6 Doc Reviewer Deferred-fix-incomplete cited sites; **Phase 6 four-dimensional convergence DEFERRED** (Dims 1-3 not at MVR; methodology-honest declaration). Round 3 routing per Phase 4 Feedback Integration Loop. Full narrative: [Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z).
- **[Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z) Finding 5** — Round 3 inline fix cycle + 4-cluster cold-session verification (adversarial-pair separation: engineering / security+ux / red-team+technical-writer / documentation-reviewer+solution-owner). **7 of 10 domains at MVR** (SE, Security, UX, Red Team, TW, SO, VDD-IAR Alignment); 2 operator-gated (Platform Engineer install-verification + Performance Engineer fsync benchmark); 1 Deferred-carryforward (Documentation Reviewer sweep-discipline gap — routed to PR [#40](https://github.com/magnificentlycursed/guild-portfolio/pull/40) upstream-suite-remediation per operator-queued sequence). Inline fixes for 7 new Round 3 findings landed mid-round: UX F6/F7/F8 → [`src/main.rs:32-48`](../vsdd-suite-reference-examples/bookmark-cli-manual/src/main.rs) + [`src/main.rs:79-98`](../vsdd-suite-reference-examples/bookmark-cli-manual/src/main.rs) + [`manual-tests/layer-1.md:14-21`](../vsdd-suite-reference-examples/bookmark-cli-manual/manual-tests/layer-1.md); Red Team F2 → Accepted risk per [`DESIGN.md:184`](../vsdd-suite-reference-examples/bookmark-cli-manual/DESIGN.md) (load-side TOCTOU; tight `O_NOFOLLOW` fix deferred pending `libc` dep); Red Team F3 → curated `is_format_char` extended at [`src/lib.rs:330-380`](../vsdd-suite-reference-examples/bookmark-cli-manual/src/lib.rs); TW F4 → angle-bracket placeholders at [`README.md:19-20,41`](../vsdd-suite-reference-examples/bookmark-cli-manual/README.md) → UPPERCASE-KEBAB-CASE; TW F3 + F5 verified already-Resolved by prior mechanical sweep. **Phase 6 four-dimensional convergence remains DEFERRED** — Platform Engineer install-verification operator-gate is the hard ceiling. Full narrative: [Review 82](suite-development/review-log/2026-05-20-suite-review.md#review-82--2026-05-20-2000z).
- **9 per-domain Round 3 entries** at [`vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/2026-05-20-{domain-slug}.md`](../vsdd-suite-reference-examples/bookmark-cli-manual/vsdd-suite/review-log/) — appended to existing per-domain files as `## Review 3 — 2026-05-20 22:00Z` sections per the per-session-file convention; 4 intermediate cluster files consolidated + deleted.

**Backlog after Review 82: 1 Open ([Review 79 Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** (the bookmark-cli-manual findings are project-scope; they don't roll up to the suite-side registry).
...
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** [§ Three-audience design principle](suite-development/suite-development.md#three-audience-design-principle-review-80-finding-3) (new sub-section; heading renamed from "Dual-audience" in [Review 84](suite-development/review-log/2026-05-21-suite-review.md#review-84--2026-05-21-1100z) Finding 4 — original `dual-audience-design-principle-review-80-finding-3` HTML anchor preserved at the heading for backward link compatibility per [G-89](suite-development/FINDINGS-INDEX.md#g-89)) — Names the three audiences (suite developers + suite users + AI agents) audit-trail artifacts serve, the triple-coding discipline (narrative + structured-fact pairs in every load-bearing claim), and the suite-side / project-side parity rule.
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** [§ Agent-API surface](suite-development/suite-development.md#agent-api-surface-review-80-finding-3) (new sub-section, ~120 lines) — Full machine-parseable invariant documentation: Review heading pattern + regex; required preamble fields; classification sub-section vocabulary; Finding header patterns (standard, with parenthetical, errata, legacy G-XX); **NEW per-Finding anchor IDs** (`<a id="rN-fM"></a>`) forward-only from Review 80; lifecycle fields; required closers; registry row shapes (forward-only + legacy); 7-row common-agent-lookup-patterns table with concrete grep/awk idioms; stability commitment.
- **[`vsdd-suite/suite-development/FINDINGS-INDEX.md`](suite-development/FINDINGS-INDEX.md)** preamble — Three-audience declaration + cross-reference to the full Agent-API contract.
- **[`vsdd-suite/suite-development/review-log/2026-05-20-suite-review.md`](suite-development/review-log/2026-05-20-suite-review.md)** Review 80 Findings 1/2/3 — Per-Finding anchor IDs (`<a id="r80-f1">`, `<a id="r80-f2">`, `<a id="r80-f3">`) added immediately after each Finding header; matching anchors are in the forward-only registry rows.

### Added (suite-side — Documentation Reviewer role domain, [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 1)

### Added (suite-side — Documentation Reviewer role domain, [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 1)
- **[`vsdd-suite/domains/role/DOCUMENTATION-REVIEWER-REVIEW.md`](domains/role/DOCUMENTATION-REVIEWER-REVIEW.md)** (now tracked, ~65 lines after registration edits) — Adversarial cold-reader pair to [Technical Writer](domains/role/TECHNICAL-WRITER-REVIEW.md). 11 dimensions covering clone-and-follow fidelity, implicit-knowledge audit, forward-reference safety, cross-reference resolution, audience-fit calibration, documentation rot, recovery-from-confusion, manual-test plan executability, onboarding sequencing, manual-test file structure consistency, and **NEW Dim 11** "Inline-reference clickthrough validation" — cold-reader pair to [TW Dim 13](domains/role/TECHNICAL-WRITER-REVIEW.md). Validator pair paragraph: TW validates Doc Reviewer findings; Doc Reviewer validates TW findings.
- **[`vsdd-suite/hooks/check-project-review-discipline.py`](hooks/check-project-review-discipline.py)** — `DOMAIN_CLASSIFICATIONS` dict extended with `documentation-reviewer: {Resolved, Deferred, Dismissed, Hallucinated}`. `KNOWN_DOMAIN_SLUGS` simplified to `frozenset(DOMAIN_CLASSIFICATIONS.keys())` (the special-case allowlist entry that previously held Doc Reviewer is no longer needed).
- **[`vsdd-suite/domains/DOMAIN-INDEX.md`](domains/DOMAIN-INDEX.md)** — New role-domain row between Technical Writer and Accessibility. Activation criteria: activates whenever Technical Writer activates (capstone+ intent; portfolio intent when intended for external reading). Extended-pool sentence updated to include `Documentation Reviewer`.

### Changed (suite-side — forward-link reciprocity unwound, [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 1)
- **[`vsdd-suite/domains/role/TECHNICAL-WRITER-REVIEW.md`](domains/role/TECHNICAL-WRITER-REVIEW.md)** Validator pair paragraph + Dim 13 closing paragraph — "(forthcoming)" framing removed; Doc Reviewer now named as active validator pair with cross-reference to [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z).
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** [§ Naming and identifier discipline](suite-development/suite-development.md#naming-and-identifier-discipline-review-78-finding-4) + [§ Anchor-link convention for cross-references](suite-development/suite-development.md#anchor-link-convention-for-cross-references-review-79-finding-3) — both companion-review-dimension paragraphs converted "(forthcoming)" references to active cross-references.

### Added (suite-side — GitHub render-target conventions, [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 2)
- **[`vsdd-suite/supplements/markdown.md`](supplements/markdown.md)** [§ GitHub render-target conventions](supplements/markdown.md#github-render-target-conventions) (new section, ~60 lines) — Codifies the [GitHub Docs Style Guide](https://github.com/github/docs/blob/main/content/contributing/style-guide-and-content-model/style-guide.md) subset most relevant to the suite + reference-example content: heading conventions, link text, inclusive language, voice and tense, acronym handling, GFM alerts, code blocks, tables, alt text, file names. Source-of-truth precedence declared: GitHub style guide → suite-development.md § Naming → suite-development.md § Anchor-link convention → this supplement.
- **[`vsdd-suite/supplements/markdown.md`](supplements/markdown.md)** § Baseline standards — New first bullet declaring GitHub as the canonical render target with reference to the GitHub style guide.

### Changed (reference example — `vsdd-suite-reference-examples/bookmark-cli-manual/`, [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 1)
- **`DESIGN.md`** § Project intent — Active domain set updated: **10 role + 1 meta = 11** → **11 role + 1 meta = 12** ([Documentation Reviewer](../vsdd-suite/domains/role/DOCUMENTATION-REVIEWER-REVIEW.md) added to capstone-tier extended domains). Anchor-link convention applied to the active-domain-set declaration.
- **`vsdd-suite/DOCUMENTATION-REVIEWER-REVIEW.md`** (new, project per-domain index stub) — Reviews-table empty; rounds populate when the cold-session Doc Reviewer round runs as part of the queued bookmark-cli-manual 6-phase IAR execution.

### Resolved
- **[Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 1** — Documentation Reviewer role domain registered as TW cold-reader pair. Full narrative: [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z).
- **[Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 2** — GitHub Docs Style Guide adopted as canonical render-target style authority + codification subset authored in `supplements/markdown.md` § GitHub render-target conventions. Full narrative: [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z).
- **[Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z) Finding 3** — Triple-audience design principle (suite developers + suite users + AI agents) codified + comprehensive Agent-API surface section authored + per-Finding anchor IDs added forward-only. Full narrative: [Review 80](suite-development/review-log/2026-05-20-suite-review.md#review-80--2026-05-20-1830z).

**Backlog after Review 80: 1 Open ([Review 79 Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** (unchanged — no new findings registered this Review beyond in-session Resolved).

---

## Unreleased — 2026-05-20 17:30Z ([Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z): whitepaper-canonical name alignment + Green Gate/smoke tests analysis-and-backlog (Deferred) + markdown anchor-link convention + 6 new language supplements + AI-operator boundary policy — 5 Resolved + 1 Deferred; v0.11.1; PR [#35](https://github.com/magnificentlycursed/guild-portfolio/pull/35))

### Added (suite-side — 6 new language supplements, [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Findings 4 + 5)
- **[`vsdd-suite/supplements/markdown.md`](supplements/markdown.md)** (new file, ~215 lines) — Markdown language supplement with 9 per-domain sections (Technical Writer PRIMARY; Documentation Reviewer; Quality Engineer; Solution Architect; Platform Engineer; Security; Accessibility; UX; Localization). Anchor-link convention exemplified throughout. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 4.
- **[`vsdd-suite/supplements/html.md`](supplements/html.md)** (new file, ~195 lines) — HTML language supplement with [Accessibility](domains/role/ACCESSIBILITY-REVIEW.md) as primary domain. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5.
- **[`vsdd-suite/supplements/css.md`](supplements/css.md)** (new file, ~190 lines) — CSS language supplement with [Accessibility](domains/role/ACCESSIBILITY-REVIEW.md) as primary domain. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5.
- **[`vsdd-suite/supplements/json.md`](supplements/json.md)** (new file, ~178 lines) — JSON language supplement with [Solution Architect](domains/role/SOLUTION-ARCHITECT-REVIEW.md) as primary domain. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5.
- **[`vsdd-suite/supplements/yaml.md`](supplements/yaml.md)** (new file, ~195 lines) — YAML language supplement with [Platform Engineer](domains/role/PLATFORM-ENGINEER-REVIEW.md) as primary domain. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5.
- **[`vsdd-suite/supplements/toml.md`](supplements/toml.md)** (new file, ~250 target lines) — TOML language supplement with [Platform Engineer](domains/role/PLATFORM-ENGINEER-REVIEW.md) as primary domain. Authored under [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5.

Suite supplement coverage now includes all 10 languages in active use across the guild-projects portfolio (Rust, TypeScript, Python, Bash, Markdown, HTML, CSS, JSON, YAML, TOML).

### Added (suite-side — anchor-link convention, [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 3)
- **[`vsdd-suite/suite-development/suite-development.md`](suite-development/suite-development.md)** [§ Anchor-link convention for cross-references](suite-development/suite-development.md#anchor-link-convention-for-cross-references-review-79-finding-3) (new sub-section) — internal-navigability table (G-IDs, Reviews, files, domains, primers, log files) + external-credit/sourceability table (governing documents, software, people) + first-mention-per-file rule.
- **[`vsdd-suite/domains/role/TECHNICAL-WRITER-REVIEW.md`](domains/role/TECHNICAL-WRITER-REVIEW.md)** Dim 13 (new) — "Inline-reference navigability ([Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 3)". Catches unlinked references in forward-facing project documentation at Phase 3 review time.
- **[`vsdd-suite/suite-development/FINDINGS-INDEX.md`](suite-development/FINDINGS-INDEX.md)** — `<a id="g-N"></a>` anchor IDs added to all 177 G-rows so prose `[G-N](FINDINGS-INDEX.md#g-N)` resolves to the registry row.

### Changed (suite-side — whitepaper-canonical naming, [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 1)
- **[`vsdd-suite/primers/2b-implementation.md`](primers/2b-implementation.md)** H1 — "Implementation" → "Minimal Implementation" (whitepaper-canonical name). Whitepaper-alignment note added.
- **[`vsdd-suite/primers/3-review-session.md`](primers/3-review-session.md)** H1 — "Adversarial Review" → "Adversarial Refinement". Whitepaper-alignment note added.
- **[`vsdd-suite/primers/4-feedback-integration.md`](primers/4-feedback-integration.md)** H1 — "Feedback Integration" → "Feedback Integration Loop". Whitepaper-alignment note added.
- **[`vsdd-suite/primers/6-convergence.md`](primers/6-convergence.md)** H1 — "Four-Dimensional Convergence" → "Convergence (VSDD Phase 6 — The Exit Signal)" (whitepaper-canonical; four-dimensional framing preserved in prose as suite specialization).
- **[`vsdd-suite/primers/2a-red-gate.md`](primers/2a-red-gate.md)** — Kept H1 as "Red Gate" (suite-specific commit-discipline name within whitepaper's "Test Suite Generation" activity); whitepaper-alignment note added clarifying both names.
- **Phase 5 hardening Title Case** swept across primer 5 + primer 6 + DOMAIN-INDEX + VDD-IAR-Alignment + README + suite-development.md + 6 bookmark-cli-manual files: "purity-boundary verification" → "Purity Boundary Audit"; "mutation testing" → "Mutation Testing"; "fuzzing" → "Fuzz Testing"; "formal proof" → "Proof Execution".

### Added (suite-side — AI-operator boundary policy, [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 6)
- **[`/.claude/settings.json`](../.claude/settings.json)** (new file, project root) — Project-scoped permission policy with `permissions.deny` rules using gitignore-anchor syntax to block Write/Edit tool calls against `//tmp/**`, `//private/tmp/**`, `//var/**`, `//etc/**`, `//usr/**`, `//opt/**`, `//System/**`, `//Library/**`. Top-of-file `_comment` field documents the policy gap (subprocess writes via Bash → Python script not caught by Write/Edit deny rules; OS-level sandboxing required for full coverage). Cross-references the [Claude Code permissions docs](https://code.claude.com/docs/en/permissions).

### Changed (suite-side — audit-trail integrity, [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 6)
- **[`vsdd-suite/suite-development/review-log/2026-05-20-suite-review.md`](suite-development/review-log/2026-05-20-suite-review.md)** — Two prose edits removing Claude-private auto-memory file (`feedback_avoid_lettering.md`) citations from public suite audit-trail prose. Audit trail is now self-contained — readers no longer need access to AI-private memory directory to follow narrative. The Review 78 Finding 3 retroactive prose edit is defensible per the G-89 carve-out for integrity defects (vs. narrative drift).

### Resolved
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 1** — Whitepaper-canonical name alignment across Phase primers. Full narrative: [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z).
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 3** — Markdown anchor-link convention for inline references (internal navigability + external credit/sourceability). Full narrative: [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z).
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 4** — Markdown language supplement authored. Full narrative: [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z).
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 5** — Language-supplement coverage gap-fill (5 new supplements). Full narrative: [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z).
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 6** — AI-operator boundary discipline: audit-trail leak cleanup + project-scoped permission policy. Full narrative: [Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z).

### Deferred
- **[Review 79](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z) Finding 2** — Green Gate + smoke tests analysis-and-backlog. Triggers: (a) explicit operator decision; (b) a second project's Phase 2b / Phase 2c experience surfaces a defect Green Gate would have caught; (c) a third-party install verification fails because no smoke-test gate caught a basic-boot regression. Auto-Backlog clause: 2026-09-30 if no trigger fires.

**Backlog after Review 79: 1 Open ([Finding 2 Deferred](suite-development/review-log/2026-05-20-suite-review.md#review-79--2026-05-20-1730z)) + 7 prior-Deferred** ([G-159](suite-development/FINDINGS-INDEX.md#g-159), [G-168](suite-development/FINDINGS-INDEX.md#g-168), [G-169](suite-development/FINDINGS-INDEX.md#g-169), [G-170](suite-development/FINDINGS-INDEX.md#g-170), [G-171](suite-development/FINDINGS-INDEX.md#g-171), [G-172](suite-development/FINDINGS-INDEX.md#g-172) unchanged + Review 76 Finding 4 bundled-Deferred).

---

## Unreleased — 2026-05-20 16:30Z (Review 78: bookmark-cli-manual capstone-intent promotion + 6-phase structural preparation + install-verification placement convention + Phase 5 hardening lettering retired + naming-discipline process — 4 Resolved; v0.11.0)

### Changed (suite-side — naming convention shift, Finding 3)
- **`vsdd-suite/primers/5-formal-hardening.md`** — All H3 section headings rewrote: "Surface A.0" / "Surface A" / "Surface B" / "Surface C" / "Surface D" retired in favor of descriptive names ("Purity-boundary verification" / "Property-based testing" / "Mutation testing" / "Fuzzing" / "Formal proof"). All inline "Surface X" references replaced with the descriptive name. `**Phase 5 surface:**` preamble tag renamed to `**Phase 5 hardening:**`. The 16 forward-facing file sweep covers primer 5 + primer 6 + DOMAIN-INDEX + VDD-IAR-Alignment dim 13 + README + suite-development.md + 10 bookmark-cli-manual files. Historical CHANGELOG / COMPATIBILITY / pre-Review-78 review-log entries preserved per G-89 — legacy G-173-G-176 registry rows retain their original "Surface A.0 / B / C / D" framings as historical narrative.

### Added (suite-side — naming-discipline process, Finding 4)
- **`vsdd-suite/domains/role/TECHNICAL-WRITER-REVIEW.md`** Dim 12 (new) — "Lettering / abbreviation lookup cost (Review 78 Finding 4)". Evaluates project documentation against the lookup-cost question: does the doc use single-letter labels as primary identifiers when descriptive names would carry the meaning? Names acceptable abbreviations (Dim N / Layer N / Round N — concept-word in the abbreviation) vs. the anti-pattern (single-letter labels next to methodology concept words). Detector pattern enumerated. Cross-references the suite-authoring discipline.
- **`vsdd-suite/suite-development/suite-development.md`** § Naming and identifier discipline (Review 78 Finding 4) (new sub-section under § Governing standard for session primers). Suite-authoring discipline: descriptive names are the primary identifier; letters/codes at most ordering aids; existing well-established abbreviations like Dim N stay; historical references preserved per G-89. Mechanical detector pattern named for audit support. Companion TW Dim 12 cross-referenced.
- **Initial mechanical scan run** across forward-facing suite content + reference example. 14 candidate matches, all inside the new TW Dim 12 + suite-development.md § Naming discipline sections (explanatory references, not orphan identifiers). No orphan letter-identifiers remain in forward-facing content.

### Resolved
- **Review 78 Finding 1** — bookmark-cli-manual capstone-intent promotion + structural preparation. Full narrative: [Review 78](suite-development/review-log/2026-05-20-suite-review.md#review-78--2026-05-20-1630z).
- **Review 78 Finding 2** — Install-verification placement convention shift. Full narrative: [Review 78](suite-development/review-log/2026-05-20-suite-review.md#review-78--2026-05-20-1630z).
- **Review 78 Finding 3** — Phase 5 hardening lettering retired in favor of descriptive names. 16 files swept; operator feedback saved as memory. Full narrative: [Review 78](suite-development/review-log/2026-05-20-suite-review.md#review-78--2026-05-20-1630z).
- **Review 78 Finding 4** — Process for finding lettering / abbreviation overuse: TW Dim 12 + suite-development.md § Naming and identifier discipline + initial scan clean. Future hook-mechanization deferred under "earned by recurrence". Full narrative: [Review 78](suite-development/review-log/2026-05-20-suite-review.md#review-78--2026-05-20-1630z).
...
### Changed (suite-side — Finding 2 PE Dim 38 + Finding 3 lettering sweep)
...
- **6 suite-side files swept** per Finding 3 (Phase 5 hardening lettering retirement): `primers/5-formal-hardening.md` (H3 headings + inline references + `**Phase 5 hardening:**` preamble tag rename), `primers/6-convergence.md` (inline references), `domains/DOMAIN-INDEX.md` (Phase 5/6 strategy declaration prose), `domains/meta/VDD-IAR-ALIGNMENT-REVIEW.md` (dim 13), `README.md` (worked-example § Phase 5 walkthrough), `suite-development/suite-development.md` (any mentions).
...
### Added
- **`vsdd-suite/suite-development/suite-development.md`** § Validation loop discipline — new sub-section (~80 lines) introducing the four lifecycle fields per finding (`**Owner:** <domain-slug>` required for non-Hallucinated; `**Status:** {raised \| assigned \| fix-landed \| validated}` required for non-Hallucinated; `**Blocked by:**` optional with `*(none)*` placeholder; `**Validator:**` required for Resolved). Sub-state transition table; strict self-validation policy (Portfolio Assessment domain-level allowlist); owner-field qualifier choice (single domain slug; no Layer qualifier — the Layer column in the FINDINGS-INDEX registry handles cross-cutting layer filtering); forward-only constraint (2026-05-21 cutoff).
- **`vsdd-suite/domains/role/*.md`** (15 role domains) + **`vsdd-suite/domains/meta/VDD-IAR-ALIGNMENT-REVIEW.md`** — each gained a `**Validator pair (Review 77):**` paragraph after the Language-and-interface-supplement line (UX's was after the Interface-type line) naming the natural cross-domain validator for findings owned by that domain. Pair mapping: Security ↔ Red Team (adversarial pair, suite's canonical worked example); TW ↔ Doc Reviewer (forward-link to subsequent Review); QE → SE (for coverage/mutation fixes) or `*self*` (test-discipline meta); SE → QE; DE → SE or PE; PE → SE or `*self*` (shift-left mechanizations); UX → SE or SO; SA → SO (Raised-to-SO subset) or `*self*` (architecture-doctrine); SO → VDD-IAR Alignment; VDD-IAR Alignment → SO; Accessibility → SE or UX; Privacy → Security or SO; Localization → SE; Performance Engineer → SE; Portfolio Assessment → `*self*` (blanket allowlist — introspective evaluation, not defects).
- **`vsdd-suite/hooks/check-project-review-discipline.py`** — new `_check_lifecycle_fields` function adds 5 checks gated on a separate forward-only threshold (2026-05-21, day-after-Review-77-adoption): (6) every non-Hallucinated finding has `**Owner:** <domain-slug>` or is under `### Raised to SO` shorthand; (7) Owner value is in the known domain-slug set; (8) every Resolved finding has `**Validator:**`; (9) Validator value is a known domain slug OR `*self*` with substantive rationale (placeholder patterns `TBD`, `N/A`, `no pair available` are rejected); (10) Status value in `{raised, assigned, fix-landed, validated}`. Portfolio Assessment blanket-allowlisted for `*self*` per the domain-level allowlist policy. Separate threshold keeps pre-Review-77 entries that comply with Review 74 from regressing under Review 77's stricter rules.

### Changed
- **`vsdd-suite/suite-development/suite-development.md`** § Per-review entry preamble — added a note that ownership/validation lifecycle fields live in the per-finding body, NOT the entry preamble.
- **`vsdd-suite/suite-development/suite-development.md`** § Finding body — structure block extended with the 4 new fields; bullet list extended with field-order rule + Hallucinated exemption + Review 77 forward-only constraint reference.
- **`vsdd-suite/templates/PROJECT-FINDINGS-INDEX-template.md`** — schema extended with Owner + Validator columns. Quick-lookup section gained two new grep examples for Owner-by-domain and Validator-by-domain filtering. New "Self-validated findings (audit-trail signal)" diagnostic grep surfaces Resolved findings that didn't get cross-domain validation. Inline HTML comment updated with Owner/Validator semantics + Review 77 forward-only constraint.
- **`vsdd-suite/suite-development/FINDINGS-INDEX.md`** § Findings registry (forward-only) — schema parallel-extended with Owner + Validator columns. Suite-side registry tracks suite-development findings that can have cross-domain Owner/Validator (e.g., a finding fixing the SE primer is Owner: software-engineer + Validator: quality-engineer).

### Added
- **`vsdd-suite/domains/meta/SANITY-CHECK-REVIEW.md`** (new, ~120 lines) — new meta domain. Primary purpose: validator-of-last-resort for findings owned by domains without a natural cross-domain pair (PE shift-left mechanizations; SA architecture-doctrine; QE test-discipline meta; Portfolio Assessment introspective dimensions; Security findings with no Red Team validation surface; TW findings pre-Doc-Reviewer-registration). Secondary purpose: rubber-ducking surface for developers articulating problems whose solution emerges in articulation. Takes DESIGN.md + architectural context as input. Meta domain (no reviewer-role persona). Classification universe: Resolved / Dismissed / Hallucinated. 8 dimensions covering coherence-with-DESIGN.md, coherence-with-architecture, internal-consistency-of-articulation, hidden-assumption-surfacing, validator-of-last-resort discipline (three-question check), rubber-duck closure honesty, spec-drift detection at meta level, meta-discipline integrity.
...
### Changed
- **`vsdd-suite/suite-development/suite-development.md`** § Validation loop discipline — Validator paragraph + Strict self-validation policy paragraph rewritten: the recommended path for findings without a cross-domain pair is `**Validator:** sanity-check` (the meta-validator-of-last-resort), not `*self*`. `*self*` remains valid only where the work has no spec/architecture interface at all. New paragraph introducing Sanity Check meta domain with the two-purpose design + classification universe + when-it-runs guidance. Domain-level allowlist retired (Portfolio Assessment moves to using `sanity-check`).
- **`vsdd-suite/hooks/check-project-review-discipline.py`** — `DOMAIN_CLASSIFICATIONS` extended with `sanity-check` entry. `KNOWN_DOMAIN_SLUGS` consequently includes `sanity-check`. `SELF_VALIDATION_BLANKET_ALLOWLIST` reduced to empty set by default (the prior Portfolio Assessment entry is superseded by `sanity-check`). The strict-self-validation check still runs but the recommended path is `**Validator:** sanity-check`.
- **6 domain prompts** — updated to reference `sanity-check` as the validator-of-last-resort instead of `*self*`: `SECURITY-REVIEW.md` (findings with no Red Team validation surface); `QUALITY-ENGINEER-REVIEW.md` (test-discipline meta); `PLATFORM-ENGINEER-REVIEW.md` (shift-left mechanizations); `SOLUTION-ARCHITECT-REVIEW.md` (architecture-doctrine without Raised-to-SO); `PORTFOLIO-ASSESSMENT-REVIEW.md` (introspective dimensions — blanket-allowlist retired); `TECHNICAL-WRITER-REVIEW.md` (pre-Doc-Reviewer-domain-registration interim path). The 5 existing PE-authored suite hooks preserve their original `*self*` framing per G-89 forward-only.

### Resolved
- **Review 77 Finding 1** — Ownership / blocking / validation lifecycle methodology introduced. Closes the gap between the suite's existing classification-centric finding model (what a finding IS + where it routes phase-wise) and the cross-domain accountability model the operator articulated (Security ↔ Red Team adversarial pair; SA/SO leadership receiving expert advice; QE/Security raisers with other-domain fixers; PE shift-left collaboration with SE/Data/Security). Full narrative: [Review 77](suite-development/review-log/2026-05-20-suite-review.md#review-77--2026-05-20-1545z).
- **Review 77 Finding 2** — Sanity Check meta domain introduced as validator-of-last-resort + rubber-ducking surface. Surfaced mid-session by operator observation against Finding 1's strict-self-validation policy — the self-validation seam is the discipline's degradation seam; better to have a structured meta-validator for these cases than to rely on the per-finding rationale to be honest. Full narrative: [Review 77](suite-development/review-log/2026-05-20-suite-review.md#review-77--2026-05-20-1545z).

### Note
**Backlog after Review 77: 0 Open + 7 Deferred** (G-159, G-168, G-169, G-170, G-171, G-172 unchanged + Review 76 Finding 4 bundled-Deferred — no new findings registered this Review). Forward-only per G-89: lifecycle fields apply to findings dated 2026-05-21 or later; pre-cutoff findings (including Reviews 73–76 in this suite-review log and the 3 existing bookmark-cli-manual rounds) are NOT migrated by the hook's enforcement. Reference examples may migrate as part of their capstone-intent promotion under the G-177 precedent in a subsequent PR. The Documentation Reviewer ↔ TW pair is forward-linked from TW's new Validator-pair paragraph + the Python/Bash supplements' Doc Reviewer sections; the forthcoming Doc Reviewer domain registration (subsequent Review) activates the pair.
...
### Changed
- **`vsdd-suite/suite-development/FINDINGS-INDEX.md`** — opening prose dropped "(gaps)" parenthetical and "indexed by gap ID" framing; new **§ Conventions** section names the two-section structure and the cross-scope project-shape mirroring goal; **§ Adding and updating findings** rewritten to drop the new-G-ID instruction (legacy series closed; new findings identified by `Review N Finding M` anchor); new **§ Findings registry (forward-only)** section added with the project-aligned schema `\| Review \| Lens \| Finding \| Title \| Source \| Classification \| Status \| Anchor \|` and empty body (next suite review surfacing a tracked finding adds the first row); legacy `## Gap Registry` renamed to **§ Legacy registry (G-01–G-182, closed to new entries)** with column shape + row contents preserved untouched per G-89 forward-only narrative-preservation policy; trailing Status-values footer split into legacy + forward-only sets.
- **`vsdd-suite/suite-development/suite-development.md`** — § "Running gap analysis" renamed to § "Walking the findings registry"; suite-structure table row "Gap analysis log" → "Findings index" with cross-scope-consistency note; "gap registry" / "gap" / "gap ID" prose replaced with finding-style language throughout forward-facing sections (lines 3, 9, 30, 358, 371, 377, 381, 383, 385, 387, 389, 401, 426, 429, 434, 435, 437, 438, 476); the three-artifact paragraph updated "IAR suite" → "VSDD Suite". **Suite review entry format** (load-bearing): classification headings reshaped to mirror project-level set (`### Resolved` / `### Dismissed` / `### Hallucinated` / `### Open` / `### Deferred`); `### New gap registered` retired going forward (existing entries preserved as historical records per G-89); `**Finding N — Title**` is the heading form for all new findings (whether resolved in-session or registered for tracking); `**G-XX — Title**` retained as accepted form for legacy-registry walks.
- **`vsdd-suite/suite-development/README.md`** — lines 21, 42, 44 — FINDINGS-INDEX description rewritten to name the two-section structure + cross-scope project-shape mirroring; "walk all Open gaps" → "walk all Open findings"; "new gap registered" / "existing gap" / "registers a gap" → finding-style language with the no-new-ID-prefix rule called out.
...
- **`vsdd-suite/primers/3-review-session.md`** — line 150 ("New gap registrations also need a row …") → "New findings registered for tracking also need a row in `suite-development/FINDINGS-INDEX.md` (forward-only section, identified by their `Review N Finding M` anchor — no new ID prefix; the legacy `G-` series is closed) …".
- **`vsdd-suite/hooks/check-suite-review-preamble.sh`** — docstring updated to name the Review 73 convention shift: legacy `G-` series closed; new findings identified by `Review N Finding M` anchor; `### New gap registered` heading RETIRED going forward (project-aligned `### Open` / `### Deferred` replace it); historical entries using the retired heading remain valid per G-89. Validation logic unchanged — existing `**Finding N — Title**` + `**G-XX — Title**` heading-form acceptance covers both new findings (former) and legacy-anchor walks (latter); Check 5 was already advisory-grade and remains so.

### Resolved
- **Review 73 Finding 1** — Suite-development findings registry reshaped forward-only to mirror the project FINDINGS-INDEX shape; "gap analysis" / `G-XX` verbiage retired going forward. Gaps not renamed retroactively (forward-only constraint per G-89). All historical G-IDs (G-01..G-182) remain valid as anchors throughout the suite. Cross-scope consistency goal achieved: a suite contributor walking the suite-development registry now encounters the same column shape, classification universe, source field, and anchor pattern as a suite user walking a project's `FINDINGS-INDEX.md`. Full narrative: [Review 73](suite-development/review-log/2026-05-20-suite-review.md#review-73--2026-05-20-1130z).

### Note
**Backlog after Review 73: 0 Open + 6 Deferred** (G-159, G-168, G-169, G-170, G-171, G-172 — unchanged; no findings closed or newly tracked beyond the convention shift itself, which is its own audit-trail anchor). The `check-suite-review-preamble.sh` hook validates the new Review 73 entry (per-review preamble fields, finding-header forms, closer-line presence — all enforced; the retired `### New gap registered` heading enforcement was already advisory-grade and was not behavioral). Forward-only per G-89: the legacy registry section + every historical `G-XX` cross-reference throughout the suite is preserved as audit-trail records.

---

## Unreleased — 2026-05-20 10:15Z (Review 72: Phase 5/6 README integration + legacy verbiage cleanup + G-177 operator promotion — 3 Resolved; v0.7.8)

### Changed
- **`vsdd-suite/primers/5-formal-hardening.md`** § Phase 5 log format — rewrote to retire `vsdd-suite/PHASE-5-LOG.md` per-project file prescription; Phase 5 work now files under per-domain review logs with `**Phase 5 surface:**` preamble tag (Surfaces A / A.0 / D → SA log; Surfaces B / C → QE log). § Manual mode + § Completion criteria #2 + Surface C JS/TS line updated to match.
- **`vsdd-suite/primers/6-convergence.md`** § Phase 6 convergence record format — substantial rewrite: the convergence record IS the final VDD-IAR Alignment review round titled "Review N — Phase 6 four-dimensional convergence (project-terminal)" with the four-dimension attestations + cross-dimension consistency check + signed closing per the round entry format. § Crosslink mode + § Manual mode + § Completion criteria + § Anonymization-aware attestation + § Layer reference + Dimension 2 verification step + Dimension 2 disposition record + Dimension 4 signal all updated to cite per-domain rounds + the final VDD-IAR Alignment round instead of `PHASE-5-LOG.md` / `PHASE-6-CONVERGENCE.md`.
- **`vsdd-suite/domains/meta/VDD-IAR-ALIGNMENT-REVIEW.md`** dim 13 — Surface-activation check now evaluates per-domain rounds with `**Phase 5 surface:**` preamble (SA log for A/A.0/D; QE log for B/C) instead of `PHASE-5-LOG.md` per-layer entries.
- **`vsdd-suite/domains/meta/VDD-IAR-ALIGNMENT-REVIEW.md`** dim 14 — Phase 6 convergence evaluation now targets the final VDD-IAR Alignment review round (titled "Review N — Phase 6 four-dimensional convergence (project-terminal)") instead of `vsdd-suite/PHASE-6-CONVERGENCE.md`.
- **`vsdd-suite/README.md`** Quickstart sections (both crosslink-primary and manual) — added Phase 5 step 7 + Phase 6 step 8 reflecting the post-G-177 per-domain log + final VDD-IAR Alignment round pattern.
- **`vsdd-suite/README.md`** § Per-layer flow (within a project) ASCII diagram — extended with Phase 5 box (conditional, after layer-gate close criteria pass, before merge) and Phase 6 box (project-terminal, after every layer's Phase 5). Diagram annotations name G-162 strategy declarations + the per-domain log pattern for findings.
- **`vsdd-suite/README.md`** Worked example — added § Phase 5 — Formal Hardening + § Phase 6 — Four-Dimensional Convergence walkthrough sections with `[crosslink]` + `[manual]` blocks. Phase 5 section includes the four surfaces table (A / A.0 / B / C / D) with per-surface → per-domain log mapping. Phase 6 section includes the four dimensions table (Spec / Test / Implementation / Formal-verification) + the load-bearing cross-dimension consistency check explanation.
- **`vsdd-suite/suite-development/FINDINGS-INDEX.md`** H1 + opening paragraph + § "Adding and updating findings" (renamed from "How to run a gap analysis") — "IAR Suite Gap Analysis Log" → "VSDD Suite Findings Index"; "gap analysis runs against the IAR suite" → "findings registry against the VSDD Suite"; section body rewritten to point at `suite-development.md` as the canonical workflow source (single-source-of-truth) rather than re-stating the workflow. Pre-merge follow-up sweep also reworded the § Gap Registry preamble line "Each ID links to the gap analysis run where it was first identified" → "Each ID links to the suite review entry where the gap was first identified" (missed in the initial G-182 sweep; caught by grep before merge).
- **`vsdd-suite/suite-development/SUITE-DEVELOPMENT-REVIEW.md`** H1 + opening paragraph — "IAR Suite Review" → "VSDD Suite Review Index"; "The IAR suite is itself a software artifact" → "The VSDD Suite is itself a software artifact" + "expanded beyond its original VSDD Phase 3 (IAR) scope to own every VSDD phase 1a+1b through 6".
- **`vsdd-suite/suite-development/README.md`** line 3 + line 60 — "running gap analysis" → "registering and walking findings, logging suite reviews"; "Project IAR sessions" → "Project-level review sessions."

### Resolved
- **G-177 (operator-promoted from Deferred to Addressed via G-130 preemption mechanism)** — `vsdd-suite/PHASE-5-LOG.md` and `vsdd-suite/PHASE-6-CONVERGENCE.md` per-project files duplicated per-domain review logs without earning the duplication. Resolution candidate (a) applied across the suite: retire the per-project files; Phase 5 findings file under per-domain logs with `**Phase 5 surface:**` preamble; Phase 6 convergence record is the final VDD-IAR Alignment review round. bookmark-cli (the reference example) migrated: `bookmark-cli/vsdd-suite/PHASE-5-LOG.md` deleted; SA Review 1 + QE Review 2 entries gained the surface preamble tag; cross-references updated in DESIGN.md, src/lib.rs:148, and the per-domain index. Full narrative: [Review 72](suite-development/review-log/2026-05-20-suite-review.md#review-72--2026-05-20-1015z).
- **G-181** — `vsdd-suite/README.md` did not integrate Phase 5 and Phase 6 into the operational flow despite the suite owning those phases since v0.7.0. Resolved by adding Quickstart steps 7 + 8 (both modes), extending the Per-layer flow diagram, and adding Worked example walkthrough sections — all using the post-G-177 per-domain log pattern.
- **G-182** — Legacy "IAR Suite" / "gap analysis" verbiage in suite-development governance files. Resolved by updating H1s, opening paragraphs, and section headers across FINDINGS-INDEX.md, SUITE-DEVELOPMENT-REVIEW.md, and suite-development/README.md. "Gap" preserved as concept-level term; "gap analysis run" retired session-type framing replaced by "suite review" with `Lens` field distinguishing modes.

### Note
**Backlog after Review 72: 0 Open + 6 Deferred** (G-159, G-168, G-169, G-170, G-171, G-172 — G-177 closed this round; G-181 + G-182 registered + resolved in-session; no new gaps). The `check-suite-review-preamble.sh` hook (Review 68) validates the new Review 72 entry. Per G-89 forward-only narrative-preservation policy: historical CHANGELOG / COMPATIBILITY / review-log entries that reference `PHASE-5-LOG.md` and `PHASE-6-CONVERGENCE.md` are preserved as audit-trail records. bookmark-cli is the reference example — it migrates, not gets a forward-only carve-out (the suite's reference must reflect current conventions). Coordinate with the bookmark-cli CHANGELOG entry "vsdd-suite v0.7.8 migration — 2026-05-20" which documents the project-side migration.
...
### Added
- **`vsdd-suite/primers/5-formal-hardening.md`** (new file) — Phase 5 primer with 4 surfaces: A (property-based testing for the purity boundary), B (Mutation Testing with per-mutant disposition), C (Fuzz Testing for parser/input-boundary surfaces), D (Proof Execution for designated pure functions — strictly optional). Each surface has named anti-patterns. Phase 5 log format `vsdd-suite/PHASE-5-LOG.md` per-project (cumulative across layers). Crosslink + manual mode integration.
...
### Deferred
- **G-135** — Cost/token meta-domain candidate. Deferred per the new G-130 deferral-trigger discipline (Review 59 promotion) — the suite eating its own cooking. **Trigger** (generalized in PR #45 per the AI Engineer verify-tool/plan/method-first discipline; the original Review 60 trigger named Claude Max specifically — preserved per G-89 forward-only narrative-preservation in the prose below): second portfolio project hits an **AI-tool/plan rate-limit or token-budget exhaustion event** (e.g., Claude Max daily message-cap recurrence, ChatGPT Plus message-cap, Claude API token-budget hard cap, Cursor / Aider / other-CLI rate-limit) AND operator decides AI-cost engineering is next priority arc. The original trigger phrasing was "second portfolio project hits Claude Max daily limit (or comparable token-budget exhaustion)" — generalized at PR #45 to be tool/plan-agnostic per the Finding 2 worked-example reasoning (the trigger is forward-facing for any future operator adopting the suite, not historical narrative). The PR-#39 AI-Engineer-domain registration was itself triggered by a Claude-Max-daily-limit recurrence on bookmark-cli-manual PR #38 — the prior cost-discipline build-out delivered on this trigger for the canonical first instance; G-135 remains open as the second-recurrence trigger. **Cost-of-deferral:** continued operator pain on cost/token telemetry; no shared discipline for cost-efficiency evaluation across projects. **Auto-Backlog clause:** if no progress by 2026-09-01 OR no second-project recurrence by 2026-09-01, auto-Backlogs per G-130 §3 mechanism. The Deferred status is the canonical demonstration of the G-130 mechanism applied to suite-development itself.
