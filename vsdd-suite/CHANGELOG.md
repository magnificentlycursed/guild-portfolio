...
- **`vsdd-suite/suite-development/FINDINGS-INDEX.md`** H1 + opening paragraph + § "Adding and updating findings" (renamed from "How to run a gap analysis") — "IAR Suite Gap Analysis Log" → "VSDD Suite Findings Index"; "gap analysis runs against the IAR suite" → "findings registry against the VSDD Suite"; section body rewritten to point at `suite-development.md` as the canonical workflow source (single-source-of-truth) rather than re-stating the workflow.
